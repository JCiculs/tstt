/*
N?o adianta mudar os creditos n?o foi você que fez...
...
Por padr?o o anti money hack vem desligado
...
bugs (2)
- anti fly n?o detecta       - corrigido
- anti skin bug n?o detecta  - corrigido
...
//define's
#define CONFIG "MF_CONFIG.ini" // Local onde as configuraç?es do anti cheat s?o salvas
#define CONFIGMONEY "MF_MONEY.ini" // Local onde o money dos players s?o salvos
...
//variaveis
static kickado[MAX_PLAYERS];// para n?o floodar o chat
...
static MORTO[MAX_PLAYERS];// para garantir que n?o vai getar a vida de mortos
...
public OnFilterScriptInit()
{
    ...
    else // se n?o existe ele vai criar-lo
    {
        INI_Create(CONFIG);
        ...
        if(damagedid != INVALID_PLAYER_ID)
        {
            if(CHECANDO[damagedid] == 0)
            {
                if(MORTO[damagedid] == 0)
                {
                    if(weaponid != 34)
                    {
                        CHECANDO[damagedid] = 1; // n?o dar bugs na hora de checar o player
                        static Float:Health,Float:Armour;
                        GetPlayerHealth(damagedid, Health);
                        GetPlayerArmour(damagedid, Armour);
                        if(Armour > 0.0)
                        {                              // \/ Para n?o dar falsas denuncias por causa do ping
        ...
        if(response)
        {
            ...
            if(listitem == 3)
            {
                ...
                else
                {
                    ...
                    antimoney = 1;
                    SendClientMessage(playerid, -1, "Se você estiver usando outro anti money hack por favor desligue para n?o cauzar conflito.");
                }
            }
            if(listitem == 4)
            {
...
{
    new Float:Z;// n?o mude para static
...
//////////////////////////////////////////////////////////////////
/*
Funs?o para ser ultilizada fora do FS
CallRemoteFunction("MF_GivePlayerMoney", "ii", playerid, quantidade);
================================================================
Funs?o para ser ultilizada dentro do FS
