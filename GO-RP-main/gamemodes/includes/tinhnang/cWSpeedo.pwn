...
//new carfuel[MAX_VEHICLES]; // connect this with your own vehicle fuel code (if you have one)
...
hook OnPlayerUpdate(playerid)
{
	...
	if(IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER && !IsAbicycle(vehicle)) // Making sure the player is in a vehicle as driver
	{
	    if(PlayerSpeedo[playerid] == 0)
	    {
			...
			new vehfuel[24];
			format(vehfuel, sizeof(vehfuel), "%.1f", VehicleFuel[vehicle]);
			...
			if ((IsVIPcar(vehicleid) || IsAdminSpawnedVehicle(vehicleid)))
			{
				format(vehiclehealth, sizeof(vehiclehealth), "Ultimate");
				PlayerTextDrawSetString(playerid, cWspeedo[playerid][8], vehiclehealth);
			}
		}
