...
#define           				MAX_FURNITURES2									9065
#define 						MAX_FURNITURES  								9066
#define                         MAX_TEXTURES_FURNITURE                          9064
#define                         MAX_LINES_IN_FURNITURE_TEXTURE                  30
#define  						FURNITURE_TEXTURE_SERVER_NAME   "GO:RP"
...
new Furniture[MAX_FURNITURES][Furniture_Data];
...
new Noithat[MAX_FURNITURES2][Furniture_Data];



new Object0[] = {1491,1502,1492,1493,1495,1500,1501,1496,1497,1504,1505,1506,1507,1523,1536,1535,1557,19302,19304,18756,11714,3440,3533,19943,1616,1622}; 
new Object1[] = {2514,2521,2528,2602,2738,19873,2515,2518,2523,2524,11709,2097,2516,2519,2522,2526,11732,2517,2520,2527};
new Object2[] = {2196,2238,2726,3534,921,1215,1734,15050,2069,2073,2075,2989,3526,3785,14527,19279,19121,19122,19123,19124,19125,19126,19127,18647,18648,18649,18650,18651,18652};
new Object3[] = {1998,1999,2008,2161,2162,2163,2164,2167,2165,2166,2183,2204,2205,2207,2208,2209,2210,2211,16378,2190,2202};
new Object4[] = {2594,2558,2559,2561,14752,2047,2048,2614,19306,2993,19307,2631,2632,11737,1828,2815,2817,2818,2833,2834,2836,2835,2841,2847,3935,3471,1736,14608,2641,2642,2643,2685,2715,2051,2055,2257,2254,949,2001,2010,2011,2244,2345,3802,3811,14804,19897,19896,19942,19944,18659,18661,18660,18665,18664,2404,2405,2406,1640,1641,1642,1643,2845,2846,1550,19086,2690,2773,18782,19054,19056,19076,19339,19346,19579,19610,19611,19609,19804,19815,19819,19835,19820,119825,19873,19878,19830,19831,11710,11711,11712,11725,11713,11705,11738,11729};
new Object5[] = {3111,19583,2589,2627,2628,2630,1985,2964,1518,1748,1749,1752,1786,2091,2093,2224,2296,2297,14604,19786,2778,2779,1719,2028,2229,2230,2231,2233,2227,18863};
new Object6[] = {2332,1742,14455,2608,2567,1740,1741,1743,2078,2204,2562,2568,2570,2573,2574,2576,19899,1281,1433,1594,1825,1827,2086,1822,2311,2313,2315,2592,2764,2799,2802,1700,1701,1771,1794,1795,1798,2298,1804,2301,2563,2566,2564,2565,11720,11731,14446,15035,15039,1369,1671,1714,1715,1704,1705,1708,1720,1722,1735,1739,2122,2123,2343,11665,11734,1702,1703,1706,1707,1712,1713,1753,1756,1760,1764,2290,11717};
new Object7[] = {19372,19373,19374,19375,19376,19377,19378,19379,19380,19381,19382,19383,19384,19385,19386,19387,19388,19389,19390,19391,19392,19393,19394,19395,19396,19397,19398,19399,19400,19401,19402,19403,19404,19405,19406,19407,19408,19409,19410,19411,19412,19413,19414,19415,19416};
// Texture
...
new FurnitureTexture[][E_FURNITURE_TEXTURE_DATA] = 
{
	{ 0, "INVALID", "INVALID" },
...
hook OnPlayerEditDynObject(playerid, objectid, response, Float:x, Float:y, Float:z, Float:rx, Float:ry, Float:rz){
	...
	{ 
        if(GetPVarInt(playerid, "EditorMode") == 1)
		{
			new h, furID, h2;
			if(PlayerInfo[playerid][pPhousekey] != INVALID_HOUSE_ID) h = PlayerInfo[playerid][pPhousekey];
			if(PlayerInfo[playerid][pPhousekey2] != INVALID_HOUSE_ID) h2 = PlayerInfo[playerid][pPhousekey2];
			if(GetPlayerVirtualWorld(playerid) == HouseInfo[h][hIntVW])
			{
				furID = TaoFurniture(playerid,h,oModel[playerid], x, y, z, rx, ry, rz);
				if(furID != -1)
				{
					AddFurniture(furID);
				}	
			}
			else if(GetPlayerVirtualWorld(playerid) == HouseInfo[h2][hIntVW])
			{
				furID = TaoFurniture(playerid,h2,oModel[playerid], x, y, z, rx, ry, rz);
				if(furID != -1)
				{
					AddFurniture(furID);
				}
			...
			else 
			{
				...
				{
					SCM(playerid, COLOR_LIGHTRED, "Chi co VIP Member moi co the dat noi that ben ngoai House.");
				}
				else 
				{
					furID = TaoFurniture(playerid,h,oModel[playerid], x, y, z, rx, ry, rz);
		    ...
		    DeletePVar(playerid, "FurnEditH");
			DeletePVar(playerid,"EditorMode");
			DeletePVar(playerid, "FurnID");
		}	
		if(GetPVarInt(playerid, "EditorMode") == 2)
		{
			new h = EditNoiThat[playerid];
			//HouseInfo[houseid][hoID][h] = dini_Int(HousePath(houseid), string);
			new fquery[400]; 
			HienTextDraw(playerid, "~w~Ban da luu vi tri noi that thanh cong");
			Noithat[h][fPos][0] = x;
			Noithat[h][fPos][1] = y;
			Noithat[h][fPos][2] = z;
			Noithat[h][fRot][0] = rx;
			Noithat[h][fRot][1] = ry;
			Noithat[h][fRot][2] = rz;
			Noithat[h][fVw] = GetPlayerVirtualWorld(playerid);
			Noithat[h][fInt] = GetPlayerInterior(playerid);
			Noithat[h][fDoor] = 0;
			Noithat[h][fText] = 0;
			
			SetDynamicObjectPos(Noithat[h][furnitureObject], x, y, z);
			SetDynamicObjectRot(Noithat[h][furnitureObject], rx, ry, rz);
			
			format(fquery, sizeof(fquery), "UPDATE `noithat` SET PosX = %f, PosY = %f, PosZ = %f, RotX = %f, RotY = %f, RotZ = %f WHERE `id` = %d",
				x, y, z, rx, ry, rz, Noithat[h][fID]);
			mysql_function_query(MainPipeline, fquery, false, "OnQueryFinish", "i", SENDDATA_THREAD);
    ...
    if(response == EDIT_RESPONSE_CANCEL)
	{
		...
		{
			DestroyDynamicObject(object[playerid]);
			new nt = GetPVarInt(playerid, "FurnEditH");
			object[playerid] = INVALID_OBJECT_ID;
			oModel[playerid] = 0;
		    Noithat[nt][fModel] = 0;
			SaveFurniture(nt);
		    HienTextDraw(playerid, "~w~Ban da dung viec thay doi vi tri noi that");
		    DeletePVar(playerid, "FurnID");
			DeletePVar(playerid,"EditorMode");
			DeletePVar(playerid, "FurnEditH");
		}	
...
stock LoadFurniture(furid)
{
	new string[128];
	printf("[Load Furniture] Loading noi that %d vao trong du lieu.", furid);
	format(string, sizeof(string), "SELECT * FROM `noithat` WHERE `id` = %d", furid); // Array starts at zero, MySQL starts at one.
	mysql_function_query(MainPipeline, string, true, "OnLoadFur", "i", furid);
}
...
hook OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) {
		...
		case 129:
		{
			if(response)
			{
				if(strfind(inputtext, "%", true) != -1) {
					return ShowPlayerDialog(playerid, 129, DIALOG_STYLE_INPUT, "Noi that", "Nhap noi that ban muon reset xuong ben duoi:", "Chon", "Dong");
				}
				if(strfind(inputtext, "c", true) != -1) {
					return ShowPlayerDialog(playerid, 129, DIALOG_STYLE_INPUT, "Noi that", "Nhap noi that ban muon reset xuong ben duoi:", "Chon", "Dong");
				}
				new amount = strval(inputtext);
				if(Noithat[amount][fModel] == 0) return SCM(playerid, COLOR_LIGHTRED, "[ ! ] {FFFFFF}Noi that ban lua chon khong hop le.");
				if(Noithat[amount][fHouseID] != PlayerInfo[playerid][pPhousekey] && Noithat[amount][fHouseID] != PlayerInfo[playerid][pPhousekey2])
				{
					SCM(playerid, COLOR_LIGHTRED, "ID noi that nay khong phai cua ban.");
					return 1;
				}
				LoadFurniture(amount);
			}
		}
		case 128:
		{
			...
			{
				if(strfind(inputtext, "%", true) != -1) {
					return ShowPlayerDialog(playerid, 128, DIALOG_STYLE_INPUT, "Noi that", "Nhap noi that ban muon chinh sua xuong ben duoi:", "Chon", "Dong");
				}
				if(strfind(inputtext, "c", true) != -1) {
					return ShowPlayerDialog(playerid, 128, DIALOG_STYLE_INPUT, "Noi that", "Nhap noi that ban muon chinh sua xuong ben duoi:", "Chon", "Dong");
				}
				...
				ShowPlayerDialog(playerid, 127, DIALOG_STYLE_LIST, "Chinh sua noi that", "Vi tri\nXoa", "Chon","Dong");
				EditNoiThat[playerid] = amount;
			}
		}
		case 127:
        {
            if(response)
            {
                //new key = h_ID[playerid];
                new nt = EditNoiThat[playerid];
                switch(listitem)
                {
                    case 0: // Edit Object
                    {
                    	//new obj = 0;
                        //obj = CreatePlayerObject(playerid, HouseInfo[key][hoID][nt], HouseInfo[key][hoX][nt], HouseInfo[key][hoY][nt], HouseInfo[key][hoZ][nt], 0.0, 0.0, HouseInfo[key][hoA][nt], 100.0);
                        EditDynamicObject(playerid, Noithat[nt][furnitureObject]);
                        //SetPVarInt(playerid, "FurnObject", obj);
			            SetPVarInt(playerid, "EditorMode", 2);
                    }
                    case 1: // Remove Object
                    {
                        DestroyDynamicObject(Noithat[nt][furnitureObject]);
                        format(string, sizeof(string), "Ban da xoa noi that (ID: %d, Model: %d).", nt, Noithat[nt][fModel]);
		                SendClientMessage(playerid,COLOR_WHITE, string);
		           	 	//Furniture[nt][fExists] = 0;
		           	 	Noithat[nt][fModel] = 0;
						Noithat[nt][fPos][0] = 0.0;
						Noithat[nt][fPos][1] = 0.0;
						Noithat[nt][fPos][2] = 0.0;
						Noithat[nt][fRot][0] = 0;
						Noithat[nt][fRot][1] = 0;
						Noithat[nt][fRot][2] = 0;
						Noithat[nt][fVw] = 0;
						...
						Noithat[nt][fDoor] = 0;
						Noithat[nt][fText] = 0;
						Noithat[nt][fHouseID] = 0;
		           	 	SaveFurniture(nt);    	
                    }
                    case 2:
                    {
                    	ShowPlayerDialog(playerid, 10081, DIALOG_STYLE_LIST, "Texture","Texture co san\nChon tham khao", "Chon", "Dong");
    ...
    if(dialogid == 9671)
	{
		...
		{
			if(listitem == 0)
			{ // 3109,2955,2873,1567,1493,1522,2949,1498,1536,1535,1497,1496,1506,1507
				new List:skins = list_new();
                for(new i = 0; i < sizeof(Object0); i++)
                {
                    if(Object0[i] != 0)
                    {   
                        AddModelMenuItem(skins, Object0[i]);
                    }
                } 
				ShowModelSelectionMenu(playerid, "Cua", MODEL_NOITHAT, skins);
			}
	        if(listitem == 1)
			{
				new List:skins = list_new();
                for(new i = 0; i < sizeof(Object1); i++)
                {
                    if(Object1[i] != 0)
                    {   
                        AddModelMenuItem(skins, Object1[i]);
                    }
                } 
				ShowModelSelectionMenu(playerid, "Noi that", MODEL_NOITHAT, skins);
			}
			if(listitem == 2)
			{
				new List:skins = list_new();
                for(new i = 0; i < sizeof(Object2); i++)
                {
                    if(Object2[i] != 0)
                    {   
                        AddModelMenuItem(skins, Object2[i]);
                    }
                } 
				ShowModelSelectionMenu(playerid, "Noi that", MODEL_NOITHAT, skins);
			}
			...
			{
				new List:skins = list_new();
                for(new i = 0; i < sizeof(Object3); i++)
                {
                    if(Object3[i] != 0)
                    {   
                        AddModelMenuItem(skins, Object3[i]);
                    }
                } 
				ShowModelSelectionMenu(playerid, "Noi that", MODEL_NOITHAT, skins);
			}
			...
			{
				new List:skins = list_new();
                for(new i = 0; i < sizeof(Object4); i++)
                {
                    if(Object4[i] != 0)
                    {   
                        AddModelMenuItem(skins, Object4[i]);
                    }
                } 
				ShowModelSelectionMenu(playerid, "Noi that", MODEL_NOITHAT, skins);
			}
			...
			{
				new List:skins = list_new();
                for(new i = 0; i < sizeof(Object5); i++)
                {
                    if(Object5[i] != 0)
                    {   
                        AddModelMenuItem(skins, Object5[i]);
                    }
                } 
				ShowModelSelectionMenu(playerid, "Noi that", MODEL_NOITHAT, skins);
			}
			...
			{
				new List:skins = list_new();
                for(new i = 0; i < sizeof(Object6); i++)
                {
                    if(Object6[i] != 0)
                    {   
                        AddModelMenuItem(skins, Object6[i]);
                    }
                } 
				ShowModelSelectionMenu(playerid, "Noi that", MODEL_NOITHAT, skins);
			}
			...
			{
				new List:skins = list_new();
                for(new i = 0; i < sizeof(Object7); i++)
                {
                    if(Object7[i] != 0)
                    {   
                        AddModelMenuItem(skins, Object7[i]);
                    }
                } 
				ShowModelSelectionMenu(playerid, "Noi that", MODEL_NOITHAT, skins);
			}
			if(listitem == 8){
				ShowPlayerDialog(playerid, 9672, DIALOG_STYLE_INPUT,"Noi that", "Vui long nhap (ID) noi that tu chon vao ben duoi:", "Chon", "Dong");
			}
		}
	}
	if(dialogid == 9672)
	{
		...
		{
			if(strfind(inputtext, "%", true) != -1) {
				return ShowPlayerDialog(playerid, 9672, DIALOG_STYLE_INPUT,"Noi that", "Vui long nhap (ID) noi that tu chon vao ben duoi:", "Chon", "Dong");
			}
			if(strfind(inputtext, "c", true) != -1) {
				return ShowPlayerDialog(playerid, 9672, DIALOG_STYLE_INPUT,"Noi that", "Vui long nhap (ID) noi that tu chon vao ben duoi:", "Chon", "Dong");
			}
			new amount = strval(inputtext);
			if(amount < 0 && PlayerInfo[playerid][pVIP] == 0) {
				return ShowPlayerDialog(playerid, 9672, DIALOG_STYLE_INPUT,"Noi that", "Vui long nhap (ID) noi that tu chon vao ben duoi:", "Chon", "Dong");
			}
			new Float:OX, Float:OY, Float:OZ;
			GetPlayerPos(playerid, OX, OY, OZ);
			//CreateDynamicObject(gSelectionFurnsTag[playerid][selectedFurn], OX+1.0, OY+1.0, OZ, 0.0, 0.0, 0.0, GetPlayerVirtualWorld(playerid), GetPlayerInterior(playerid), -1, 100.0);
			object[playerid] = CreateDynamicObject(amount, OX+1.0, OY+1.0, OZ, 0.0, 0.0, 0.0, GetPlayerVirtualWorld(playerid), GetPlayerInterior(playerid), -1, 100.0);
			SetPVarInt(playerid, "EditorMode", 1);
			EditDynamicObject(playerid, object[playerid]);
			for(new id = 1500; id < MAX_FURNITURES2; id++)
			{
				if(Noithat[id][fModel] == 0)
				{
					SetPVarInt(playerid, "FurnEditH", id);
					Noithat[id][fModel] = amount;
					oModel[playerid] = amount;
					break;
				}
			}
		}
    ...
    if(dialogid == 10081)
	{
		...
		{ 
			if(listitem == 0)
			{
				new str[1048 + 1048 + 40], index = 0, string[1028];
				new nt = EditNoiThat[playerid];
			    for(new i = index*(MAX_LINES_IN_FURNITURE_TEXTURE + 1); i < sizeof(FurnitureTexture); i++)
			    {
			    	if(FurnitureTexture[i][fT_index] == Noithat[nt][fModel])
			    	{
			    		format(str, sizeof(str), "%d\t%d\t%s\t\t%s\n", i, FurnitureTexture[i][fT_index], FurnitureTexture[i][fT_txdName], FurnitureTexture[i][fT_textureName]);
			    		strcat(string, str, sizeof(string));
			    	}	
		    	}	
		    	format(str, sizeof(str), "Index\tTexture Index\tTXD Name\t\tTexture Name\n%s", string);
		    	ShowPlayerDialog(playerid, 10080, DIALOG_STYLE_TABLIST, "Texture", str, "Chon", "Dong");
		    }
		    if(listitem == 1)
		    {
		    	ShowPlayerDialog(playerid, 10082, DIALOG_STYLE_INPUT, "Texture", "Texture de xuat tu 1 den 300, ban vui long nhap tung cai de tham khao cung duoc:", "Nhap", "Dong");
		    }	
	    }	
	}
	if(dialogid == 10082)
	{
		if(response)
		{
			...
			new nt = EditNoiThat[playerid];
			DestroyDynamicObject(Noithat[nt][furnitureObject]);
			Noithat[nt][furnitureObject] = CreateDynamicObject(Noithat[nt][fModel], Noithat[nt][fPos][0], Noithat[nt][fPos][1], Noithat[nt][fPos][2], Noithat[nt][fRot][0], 
			Noithat[nt][fRot][1], Noithat[nt][fRot][2], Noithat[nt][fVw], Noithat[nt][fInt]);
			Noithat[nt][fText] = amount;
			SetDynamicObjectMaterial(Noithat[nt][furnitureObject], 0, FurnitureTexture[amount][fT_index], FurnitureTexture[amount][fT_txdName], FurnitureTexture[amount][fT_textureName], 0x00000000);
			SaveFurniture(nt);
		}
	}
	if(dialogid == 10080)
	{
		if(response)
		{
			...
			Noithat[nt][furnitureObject] = CreateDynamicObject(Noithat[nt][fModel], Noithat[nt][fPos][0], Noithat[nt][fPos][1], Noithat[nt][fPos][2], Noithat[nt][fRot][0], 
			Noithat[nt][fRot][1], Noithat[nt][fRot][2], Noithat[nt][fVw], Noithat[nt][fInt]);
			Noithat[nt][fText] = listitem;
			SetDynamicObjectMaterial(Noithat[nt][furnitureObject], 0, FurnitureTexture[listitem][fT_index], FurnitureTexture[listitem][fT_txdName], FurnitureTexture[listitem][fT_textureName], 0x00000000);
			SaveFurniture(nt);
			new str[1048 + 1048 + 40], index = 0, string[1028];
		    for(new i = index*(MAX_LINES_IN_FURNITURE_TEXTURE + 1); i < sizeof(FurnitureTexture); i++)
		    {
...
forward TaoFurniture(playerid, houseid, modelid, Float:x, Float:y, Float:z, Float:rx, Float:ry, Float:rz);
public TaoFurniture(playerid, houseid, modelid, Float:x, Float:y, Float:z, Float:rx, Float:ry, Float:rz)
{
	new id = GetPVarInt(playerid, "FurnEditH");
	...
	if(Noithat[id][fModel] == oModel[playerid])
	{
		if(GetPlayerVirtualWorld(playerid) == 0)
		{
			if(!IsPlayerInRangeOfPoint(playerid, 50.0, HouseInfo[house][hExteriorX],HouseInfo[house][hExteriorY], HouseInfo[house][hExteriorZ]) && PlayerInfo[playerid][pVIP] < 2)
			{
				HienTextDraw(playerid, "Ban khong dung trong pham vi ngoi nha cua minh hoac khong co VIP");
				DestroyDynamicObject(object[playerid]);
				object[playerid] = INVALID_OBJECT_ID;
				oModel[playerid] = 0;
				Noithat[id][fModel] = 0;
		...
		if(PlayerInfo[playerid][pGo] < 20)
		{
			SCM(playerid, COLOR_LIGHTRED, " Ban khong co nguyen lieu Go, vui long mua tu nguoi khac (1 Object - 20 Go).");
			DestroyDynamicObject(object[playerid]);
			object[playerid] = INVALID_OBJECT_ID;
			oModel[playerid] = 0;
			Noithat[id][fModel] = 0;
		...
		Noithat[id][fModel] = modelid;
		Noithat[id][fHouseID] = houseid;
		Noithat[id][fID] = id;
				
		Noithat[id][fPos][0] = x;
		Noithat[id][fPos][1] = y;
		Noithat[id][fPos][2] = z;
		Noithat[id][fVw] = GetPlayerVirtualWorld(playerid);
		Noithat[id][fInt] = GetPlayerInterior(playerid);
		Noithat[id][fRot][0] = rx;
		Noithat[id][fRot][1] = ry;
		Noithat[id][fRot][2] = rz;
		Noithat[id][fDoor] = 0;
		Noithat[id][fText] = 0;
		new string[128];
		format(string, sizeof(string), "Ban da mua noi that (ID: %d, Model: %d)", id,modelid);
		SCM(playerid, COLOR_WHITE, string);
		PlayerInfo[playerid][pGo] -= 20;
		return id;
	}		
	//Furniture[id][fExists] = true;		
	return -1;
}

forward AddFurniture(id);
public AddFurniture(id)
{
	SaveFurniture(id);
	CreateFur(id);
	return true;
...
forward SaveFurniture(id);
public SaveFurniture(id)
{
	new string[1024];
	format(string, sizeof(string), "UPDATE `noithat` SET \
		`Model`=%d, \
		`PosX`=%f, \
		`PosY`=%f, \
		`PosZ`=%f, \
	...
	printf("Luu noi that thanh cong [ID: %d, Object: %d]", id, Noithat[id][fModel]);
	return true;
}
...
{
	new string[1024];
	format(string, sizeof(string), "UPDATE `furnitures` SET \
		`Model`=%d, \
		`PosX`=%f, \
		`PosY`=%f, \
		`PosZ`=%f, \
		...
		Furniture[id][fDoor],
		Furniture[id][fText],
		Furniture[id][fHouseID],
		id);
	//mysql_tquery(MainPipeline, szMiscArray, "OnSaveHouse", "ii", houseid, 0);
	mysql_function_query(MainPipeline, string, false, "OnQueryFinish", "i", SENDDATA_THREAD);
	printf("Luu noi that thanh cong [ID: %d, Object: %d]", id, Furniture[id][fModel]);
	return true;
}
...
public LoadFurnitures()
{
	...
	while(i < 5000)
	{
		cache_get_field_content(i, "id", tmp, MainPipeline); Furniture[i][fID] = strval(tmp);
			
		cache_get_field_content(i, "HouseID", tmp, MainPipeline); Furniture[i][fHouseID] = strval(tmp);
		cache_get_field_content(i, "Model", tmp, MainPipeline); Furniture[i][fModel] = strval(tmp);
		//cache_get_field_content(i, "Exists", tmp, MainPipeline); Furniture[i][fExists]= strval(tmp);
		cache_get_field_content(i, "Vw", tmp, MainPipeline); Furniture[i][fVw] = strval(tmp);
		cache_get_field_content(i, "Int", tmp, MainPipeline); Furniture[i][fInt] = strval(tmp);
		cache_get_field_content(i, "Door", tmp, MainPipeline); Furniture[i][fDoor] = strval(tmp);
		cache_get_field_content(i, "Text", tmp, MainPipeline); Furniture[i][fText] = strval(tmp);

		cache_get_field_content(i, "PosX", tmp, MainPipeline); Furniture[i][fPos][0] = floatstr(tmp);
		cache_get_field_content(i, "PosY", tmp, MainPipeline); Furniture[i][fPos][1] = floatstr(tmp);
		cache_get_field_content(i, "PosZ", tmp, MainPipeline); Furniture[i][fPos][2] = floatstr(tmp);
				
		cache_get_field_content(i, "RotX", tmp, MainPipeline); Furniture[i][fRot][0] = floatstr(tmp);
		cache_get_field_content(i, "RotY", tmp, MainPipeline); Furniture[i][fRot][1] = floatstr(tmp);
		cache_get_field_content(i, "RotZ", tmp, MainPipeline); Furniture[i][fRot][2] = floatstr(tmp);
	...
	if(i > 0)
	{
		if(i > 0) printf("[LoadFurnitures] %d noi that da duoc load.", i);
	}
	return 1;

///	printf("%d Furniture loaded", i);
...
public MyFurniture()
{
    mysql_function_query(MainPipeline, "SELECT * FROM `furnitures`", true, "LoadFurnitures", "");
	mysql_function_query(MainPipeline, "SELECT * FROM `noithat`", true, "LoadFur", "");
}
...
public LoadFur()
{
	...
	while(i < 5000)
	{
		cache_get_field_content(i, "id", tmp, MainPipeline); Noithat[i][fID] = strval(tmp);
			
		cache_get_field_content(i, "HouseID", tmp, MainPipeline); Noithat[i][fHouseID] = strval(tmp);
		cache_get_field_content(i, "Model", tmp, MainPipeline); Noithat[i][fModel] = strval(tmp);
		//cache_get_field_content(i, "Exists", tmp, MainPipeline); Furniture[i][fExists]= strval(tmp);
		cache_get_field_content(i, "Vw", tmp, MainPipeline); Noithat[i][fVw] = strval(tmp);
		cache_get_field_content(i, "Int", tmp, MainPipeline); Noithat[i][fInt] = strval(tmp);
		cache_get_field_content(i, "Door", tmp, MainPipeline); Noithat[i][fDoor] = strval(tmp);
		cache_get_field_content(i, "Text", tmp, MainPipeline); Noithat[i][fText] = strval(tmp);

		cache_get_field_content(i, "PosX", tmp, MainPipeline); Noithat[i][fPos][0] = floatstr(tmp);
		cache_get_field_content(i, "PosY", tmp, MainPipeline); Noithat[i][fPos][1] = floatstr(tmp);
		cache_get_field_content(i, "PosZ", tmp, MainPipeline); Noithat[i][fPos][2] = floatstr(tmp);
				
		cache_get_field_content(i, "RotX", tmp, MainPipeline); Noithat[i][fRot][0] = floatstr(tmp);
		cache_get_field_content(i, "RotY", tmp, MainPipeline); Noithat[i][fRot][1] = floatstr(tmp);
		cache_get_field_content(i, "RotZ", tmp, MainPipeline); Noithat[i][fRot][2] = floatstr(tmp);
...
CreateFurnitures(furid)
{
	if(IsValidDynamicObject(Furniture[furid][furnitureObject])) DestroyDynamicObject(Furniture[furid][furnitureObject]);
	Furniture[furid][furnitureObject] = CreateDynamicObject(Furniture[furid][fModel], Furniture[furid][fPos][0], Furniture[furid][fPos][1], Furniture[furid][fPos][2], Furniture[furid][fRot][0], 
	Furniture[furid][fRot][1], Furniture[furid][fRot][2], Furniture[furid][fVw], Furniture[furid][fInt]); 
	Furniture[furid][fDoor] = 0;
	if(Furniture[furid][fText] != 0)
	{
		new listitem = Furniture[furid][fText];
		SetDynamicObjectMaterial(Furniture[furid][furnitureObject], 0, FurnitureTexture[listitem][fT_index], FurnitureTexture[listitem][fT_txdName], FurnitureTexture[listitem][fT_textureName], 0x00000000);
	}
}

CreateFur(furid)
{
	if(IsValidDynamicObject(Noithat[furid][furnitureObject])) DestroyDynamicObject(Noithat[furid][furnitureObject]);
	Noithat[furid][furnitureObject] = CreateDynamicObject(Noithat[furid][fModel], Noithat[furid][fPos][0], Noithat[furid][fPos][1], Noithat[furid][fPos][2], Noithat[furid][fRot][0], 
	Noithat[furid][fRot][1], Noithat[furid][fRot][2], Noithat[furid][fVw], Noithat[furid][fInt]); 
	Noithat[furid][fDoor] = 0;
	if(Noithat[furid][fText] != 0)
	{
		new listitem = Noithat[furid][fText];
		SetDynamicObjectMaterial(Noithat[furid][furnitureObject], 0, FurnitureTexture[listitem][fT_index], FurnitureTexture[listitem][fT_txdName], FurnitureTexture[listitem][fT_textureName], 0x00000000);
	}
}
...
CMD:noithat(playerid, params[])
{
	if(PlayerInfo[playerid][pPhousekey] != INVALID_HOUSE_ID || PlayerInfo[playerid][pPhousekey2] != INVALID_HOUSE_ID)
	{
		...
		/*if(GetPlayerVirtualWorld(playerid) != 0)
		{
			if(GetPlayerVirtualWorld(playerid) != HouseInfo[h][hIntVW]) return HienTextDraw(playerid, "Ban khong o trong ngoi nha cua minh");
		}*/
		//if(GetPlayerSQLId(playerid) != HouseInfo[h][hOwnerID]) return HienTextDraw(playerid, "Ban khong phai chu so huu ngoi nha, khong the trang tri noi that"); 
		new chon[32];
	    if(sscanf(params, "s[32]", chon))
		{
			SendClientMessageEx(playerid, COLOR_LIGHTRED, "SU DUNG:{FFFFFF} /noithat [lua chon]");
			SCM(playerid, COLOR_GREY, "Lua chon: list, mua.");
			return 1;
		} // 	
		if(strcmp(chon, "mua", true) == 0)
		{
			if(GetPlayerVirtualWorld(playerid) == HouseInfo[h][hIntVW])
			{
				new dem;
				...
				{
					if(Noithat[i][fHouseID] == h)
					{
						if(Noithat[i][fModel] != 0)
						{
							dem++;
						}
					}
				...
				if(PlayerInfo[playerid][pVIP] == 0)
				{
					if(dem > 70)
					{
						SCM(playerid, COLOR_LIGHTRED, "Chi co VIP Members tro len moi co the dat hon 70 noi that.");
						return 1;
					}
					ShowPlayerDialog(playerid, 9671, DIALOG_STYLE_LIST, "Noi that", "Cua\nPhong tam\nAnh sang\nPhong lam viec\nTrang tri\nDo dien tu\nBan ghe\nWalls\nNhap (ID) noi that", "Chon", "Dong");
				}
				else 
				{
					ShowPlayerDialog(playerid, 9671, DIALOG_STYLE_LIST, "Noi that", "Cua\nPhong tam\nAnh sang\nPhong lam viec\nTrang tri\nDo dien tu\nBan ghe\nWalls\nNhap (ID) noi that", "Chon", "Dong");
			...
			{
				if(GetPlayerVirtualWorld(playerid) != 0) return HienTextDraw(playerid, "Ban khong dung trong ngoi nha cua minh");
				ShowPlayerDialog(playerid, 9671, DIALOG_STYLE_LIST, "Noi that", "Door\nRandom\nNew Add\nNhap ID", "Chon", "Dong");
			}
			else return HienTextDraw(playerid, "Ban khong dung trong ngoi nha cua minh");
			//SCM(playerid, COLOR_LIGHTRED, "He thong noi that se duoc cap nhat vao phien ban SAG v2.2.6.");
		}
		if(strcmp(chon, "list", true) == 0)
		{	
			if(GetPlayerVirtualWorld(playerid) == HouseInfo[h][hIntVW])
			{
				new slot = 0;
                new List:noithat = list_new();
				for(new i = 0; i < MAX_FURNITURES; i++)
				{
					if(Noithat[i][fHouseID] == h)
					{
						if(Noithat[i][fModel] != 0)
						{
							ChonNoiThat[playerid][slot] = Noithat[i][fModel];
							IDNoiThat[playerid][slot] = i;
                            AddModelMenuItem(noithat, Noithat[i][fModel]);
							slot++;
						}
					}
				}				
				ShowModelSelectionMenu(playerid, "Noi that", MODEL_LISTNOITHAT, noithat);
			}	
			else if(GetPlayerVirtualWorld(playerid) == HouseInfo[h2][hIntVW])
			{
				new slot = 0;
...
CMD:anoithat(playerid, params[])
{
	if(PlayerInfo[playerid][pAdmin] > 0)
	{
		new chon[32], nt;
	    if(sscanf(params, "s[32]d", chon, nt))
		{
			SendClientMessageEx(playerid, COLOR_LIGHTRED, "SU DUNG:{FFFFFF} /noithat [lua chon] [noi that]");
			SCM(playerid, COLOR_GREY, "Lua chon: move, xoa, ktra.");
			return 1;
		}
		if(strcmp(chon, "ktra", true) == 0)
		{
			new string[128];
			format(string, sizeof(string), "SELECT * FROM `noithat` WHERE `id` = %d", nt); // Array starts at zero, MySQL starts at one.
			mysql_function_query(MainPipeline, string, true, "OnLoadFur", "i", nt);
			format(string, sizeof(string), "Noi that %d (Model %d)", nt, Noithat[nt][fModel]);
			SCM(playerid, COLOR_WHITE, string);
		}
		if(strcmp(chon, "check", true) == 0)
		{
			new string[128];
			for(new i; i < MAX_FURNITURES; i++)
			{
				if(Noithat[i][fModel] != 0 && Noithat[i][fVw] == GetPlayerVirtualWorld(playerid))
				{
					if(IsPlayerInRangeOfPoint(playerid, nt, Noithat[i][fPos][0], Noithat[i][fPos][1], Noithat[i][fPos][2]))
					{
						format(string, sizeof(string), "(Du lieu 2) Noi that ID %d (Model %d) - House ID: %d", i, Noithat[i][fModel], Noithat[i][fHouseID]);
						SCM(playerid, COLOR_WHITE, string);
					}
				}
				if(Furniture[i][fModel] != 0 && Furniture[i][fVw] == GetPlayerVirtualWorld(playerid))
				{
					if(IsPlayerInRangeOfPoint(playerid, nt, Furniture[i][fPos][0], Furniture[i][fPos][1], Furniture[i][fPos][2]))
					{
						format(string, sizeof(string), "(Du lieu 1) Noi that ID %d (Model %d) - House ID: %d", i, Furniture[i][fModel], Furniture[i][fHouseID]);
						SCM(playerid, COLOR_WHITE, string);
					}
				}
			}			
		}
		if(strcmp(chon, "reset", true) == 0)
		{
			DestroyDynamicObject(Noithat[nt][furnitureObject]);
			Noithat[nt][furnitureObject] = CreateDynamicObject(Noithat[nt][fModel], Noithat[nt][fPos][0], Noithat[nt][fPos][1], Noithat[nt][fPos][2], Noithat[nt][fRot][0], 
			Noithat[nt][fRot][1], Noithat[nt][fRot][2], Noithat[nt][fVw], Noithat[nt][fInt]); 
			if(Noithat[nt][fText] != 0)
			{
				new listitem = Noithat[nt][fText];
				SetDynamicObjectMaterial(Noithat[nt][furnitureObject], 0, FurnitureTexture[listitem][fT_index], FurnitureTexture[listitem][fT_txdName], FurnitureTexture[listitem][fT_textureName], 0x00000000);
			}
		}
		if(strcmp(chon, "goto", true) == 0)
		{
			//DestroyDynamicObject(Furniture[nt][furnitureObject]);
			SetPlayerPos(playerid, Noithat[nt][fPos][0], Noithat[nt][fPos][1], Noithat[nt][fPos][2]);
			SetPlayerVirtualWorld(playerid, Noithat[nt][fVw]);
			SetPlayerInterior(playerid, Noithat[nt][fInt]);
		}	
		if(strcmp(chon, "move", true) == 0)
  		{
  			EditDynamicObject(playerid, Noithat[nt][furnitureObject]);
  			EditNoiThat[playerid] = nt;
			SetPVarInt(playerid, "EditorMode", 2);
		}
		if(strcmp(chon, "xoa", true) == 0)
		{
		    Noithat[nt][fModel] = 0;
			Noithat[nt][fPos][0] = 0.0;
			Noithat[nt][fPos][1] = 0.0;
			...
			DestroyDynamicObject(Noithat[nt][furnitureObject]);
			SaveFurniture(nt);
		    SendClientMessage(playerid,-1, " Ban da xoa noi that!");
		}	
		if(strcmp(chon, "xoacu", true) == 0)
		{
		    Furniture[nt][fModel] = 0;
			Furniture[nt][fPos][0] = 0.0;
			Furniture[nt][fPos][1] = 0.0;
			Furniture[nt][fPos][2] = 0.0;
			Furniture[nt][fRot][0] = 0;
			Furniture[nt][fRot][1] = 0;
			Furniture[nt][fRot][2] = 0;
			Furniture[nt][fVw] = 0;
			Furniture[nt][fInt] = 0;
			Furniture[nt][fHouseID] = 0;
			DestroyDynamicObject(Furniture[nt][furnitureObject]);
			SaveFur(nt);
		    SendClientMessage(playerid,-1, " Ban da xoa noi that!");
		}	
...
forward OnLoadFurniture(index);
public OnLoadFurniture(index)
{
	...
	{
		cache_get_field_content(rows, "id", tmp, MainPipeline); Furniture[index][fID] = strval(tmp);
			
		cache_get_field_content(rows, "HouseID", tmp, MainPipeline); Furniture[index][fHouseID] = strval(tmp);
		cache_get_field_content(rows, "Model", tmp, MainPipeline); Furniture[index][fModel] = strval(tmp);
		//cache_get_field_content(i, "Exists", tmp, MainPipeline); Furniture[i][fExists]= strval(tmp);
		cache_get_field_content(rows, "Vw", tmp, MainPipeline); Furniture[index][fVw] = strval(tmp);
		cache_get_field_content(rows, "Int", tmp, MainPipeline); Furniture[index][fInt] = strval(tmp);
		cache_get_field_content(rows, "Door", tmp, MainPipeline); Furniture[index][fDoor] = strval(tmp);
		cache_get_field_content(rows, "Text", tmp, MainPipeline); Furniture[index][fText] = strval(tmp);

		cache_get_field_content(rows, "PosX", tmp, MainPipeline); Furniture[index][fPos][0] = floatstr(tmp);
		cache_get_field_content(rows, "PosY", tmp, MainPipeline); Furniture[index][fPos][1] = floatstr(tmp);
		cache_get_field_content(rows, "PosZ", tmp, MainPipeline); Furniture[index][fPos][2] = floatstr(tmp);
				
		cache_get_field_content(rows, "RotX", tmp, MainPipeline); Furniture[index][fRot][0] = floatstr(tmp);
		cache_get_field_content(rows, "RotY", tmp, MainPipeline); Furniture[index][fRot][1] = floatstr(tmp);
		cache_get_field_content(rows, "RotZ", tmp, MainPipeline); Furniture[index][fRot][2] = floatstr(tmp);

		CreateFurnitures(index);
	}
	return 1;
}

forward OnLoadFur(index);
public OnLoadFur(index)
{
	new rows, fields, tmp[128];
	cache_get_data(rows, fields, MainPipeline);
	...
	for(new row; row < rows; row++)
	{
		cache_get_field_content(rows, "id", tmp, MainPipeline); Noithat[index][fID] = strval(tmp);
			
		cache_get_field_content(rows, "HouseID", tmp, MainPipeline); Noithat[index][fHouseID] = strval(tmp);
		cache_get_field_content(rows, "Model", tmp, MainPipeline); Noithat[index][fModel] = strval(tmp);
		//cache_get_field_content(i, "Exists", tmp, MainPipeline); Furniture[i][fExists]= strval(tmp);
		cache_get_field_content(rows, "Vw", tmp, MainPipeline); Noithat[index][fVw] = strval(tmp);
		cache_get_field_content(rows, "Int", tmp, MainPipeline); Noithat[index][fInt] = strval(tmp);
		cache_get_field_content(rows, "Door", tmp, MainPipeline); Noithat[index][fDoor] = strval(tmp);
		cache_get_field_content(rows, "Text", tmp, MainPipeline); Noithat[index][fText] = strval(tmp);

		cache_get_field_content(rows, "PosX", tmp, MainPipeline); Noithat[index][fPos][0] = floatstr(tmp);
		cache_get_field_content(rows, "PosY", tmp, MainPipeline); Noithat[index][fPos][1] = floatstr(tmp);
		cache_get_field_content(rows, "PosZ", tmp, MainPipeline); Noithat[index][fPos][2] = floatstr(tmp);
				
		cache_get_field_content(rows, "RotX", tmp, MainPipeline); Noithat[index][fRot][0] = floatstr(tmp);
		cache_get_field_content(rows, "RotY", tmp, MainPipeline); Noithat[index][fRot][1] = floatstr(tmp);
		cache_get_field_content(rows, "RotZ", tmp, MainPipeline); Noithat[index][fRot][2] = floatstr(tmp);
