...
stock LoaderStarting(playerid, loadingid, const LoaderInfo[], Float:speed, color) {
    ...
    TimerLoading[playerid] = SetTimerEx("LoadingProgress", 98, true, "iii", playerid,loadingid,floatround(speed));
...
public OnLoadingFinish(playerid,loadingid)
{
	switch(loadingid){
		case HAICANSA:
		{
		    if(HarvestCan[playerid] == 1)
			{
		        TogglePlayerControllable(playerid, 1);
		        ClearAnimations(playerid);
		        HarvestCan[playerid] = 0;
		        //PlayerInfo[playerid][pCansap] += 1;
		        //UpdateTextCanSa();
				//TimeHarvest[playerid] = SetTimerEx("OnPlayerHarvestMarijuana", HAICANSA*10000, 0, "ii", playerid, iMarijuana); // Timer thu hoach
  				//LoaderStarting(playerid, HAICANSA*10000, "DANG HAI CAN SA", 0.5, 10000);
			}
		}
	}
	return 1;
