...
//new Text:Bocvac_TD[1];
new PlayerText:BeKhoa_TD[MAX_PLAYERS][37];
new Text:Tieuthu_TD[1];
new Text:Danhca_TD[1];
new Text:Trucker_TD[1];
new Text:Nguoivc_TD[1];
new Text:Pizza_TD[2];
new Text:Khoangsan_TD[1];
//new PlayerText:BeKhoa_TD[MAX_PLAYERS][37];
...
new dangmuaxe;
new xemua;
new PlayerText:BanXe_TD[MAX_PLAYERS][20];
new PlayerText:ChonXe_TD[MAX_PLAYERS][2];
...
{
	new string[128];
	format(string, sizeof(string), "[GORP-Anticheats]: %s(ID: %d) Dang co the dang su dung Air-Break.", GetPlayerNameEx(playerid), playerid);
...
public OnVehicleSpawn(vehicleid) {
	...
	foreach(new i: Player)
	{
		if((v = GetPlayerVehicle(i, vehicleid)) != -1) {
			...
			new iVehicleID = CreateVehicle(PlayerVehicleInfo[i][v][pvModelId], PlayerVehicleInfo[i][v][pvPosX], PlayerVehicleInfo[i][v][pvPosY], PlayerVehicleInfo[i][v][pvPosZ], PlayerVehicleInfo[i][v][pvPosAngle],PlayerVehicleInfo[i][v][pvColor1], PlayerVehicleInfo[i][v][pvColor2], -1);
			SetVehicleHealth(iVehicleID, 250);
...
public OnVehicleMod(playerid, vehicleid, componentid) 
{
    ...
    for(new i = 0; i < sizeof(IsRim); i++)
	{
		if(IsRim[i] == componentid)
		{
		    if(!legalRims(playerid, componentid, vehicleid))
		    {
				...
				{
					new szMessage[128];
					format(szMessage, sizeof(szMessage), "[MAYCHU]: %s (ID: %d) co the dang hacking mod xe (%s (%d) %s to a %s (ID: %d))", GetPlayerNameEx(playerid), playerid, partName(componentid), componentid, partType(GetVehicleComponentType(componentid)), GetVehicleName(vehicleid), GetPlayerVehicleID(playerid));
					ABroadCast(COLOR_YELLOW, szMessage, 2);
					format(szMessage, sizeof(szMessage), "%s co the dang hacking mod xe (%s (%d) %s to a %s (ID: %d))", GetPlayerNameEx(playerid), partName(componentid), componentid, partType(GetVehicleComponentType(componentid)), GetVehicleName(vehicleid), GetPlayerVehicleID(playerid));
				...
				{
					format(string, sizeof(string), "[MAYCHU] %s da bi khoa tai khoan boi System | Ly do: Hacking God Car.", GetPlayerNameEx(playerid));
					ABroadCast(COLOR_LIGHTRED, string, 2);
					SendClientMessageEx(playerid, COLOR_LIGHTRED, string);
					PlayerInfo[playerid][pBanned] = 3;
					new playerip[32];
					GetPlayerIp(playerid, playerip, sizeof(playerip));
					format(string, sizeof(string), "[MAYCHU] %s (IP:%s) da bi khoa tai khoan boi System | ly do: Hacking God Car.", GetPlayerNameEx(playerid), playerip);
					PlayerInfo[playerid][pBanned] = 3;
	...
	if(!(1 <= GetPlayerInterior(playerid) <= 3) && !GetPVarType(playerid, "unMod")) {
		...
		if(HackingMods[playerid] < 3)		
		{
			format(szMessage, sizeof(szMessage), "[MAYCHU]: %s (ID: %d) Co the dang hack mod xe (%s (%d) %s to a %s (ID: %d))", GetPlayerNameEx(playerid), playerid, partName(componentid), componentid, partType(GetVehicleComponentType(componentid)), GetVehicleName(vehicleid), GetPlayerVehicleID(playerid));
			ABroadCast(COLOR_YELLOW, szMessage, 2);
			format(szMessage, sizeof(szMessage), "%s co the dang hack mod xe (%s (%d) %s to a %s (ID: %d))", GetPlayerNameEx(playerid), partName(componentid), componentid, partType(GetVehicleComponentType(componentid)), GetVehicleName(vehicleid), GetPlayerVehicleID(playerid));
			Log("logs/hack.log", szMessage);
			HackingMods[playerid]++;
			return 0;
		}
		...
		{
			format(string, sizeof(string), "[MAYCHU] %s da bi khoa tai khoan boi System | Ly do: Hack God Car.", GetPlayerNameEx(playerid));
			ABroadCast(COLOR_LIGHTRED, string, 2);
			SendClientMessageEx(playerid, COLOR_LIGHTRED, string);
			PlayerInfo[playerid][pBanned] = 3;
			new playerip[32];
			GetPlayerIp(playerid, playerip, sizeof(playerip));
			format(string, sizeof(string), "[MAYCHU] %s (IP:%s) da bi khoa tai khoan boi System | Ly do: Hack God Car.", GetPlayerNameEx(playerid), playerip);
			PlayerInfo[playerid][pBanned] = 3;
			Log("logs/ban.log", string);
			new ip[32];
			GetPlayerIp(playerid, ip, sizeof(ip));
...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
    ...
    if(areaid == cansa)
	{
		if(IsACop(playerid))
		{
		SendClientMessage(playerid, -1, "TINH BAO: Hien dang co nguoi hai can sa");
		}
	}
	if(areaid == hospzone[0])
	{
		SendClientMessage(playerid, -1, "Ban da vao khu vuc benh vien vui long khong gay roi trat tu neu khong muon bi xu phat");
	}
	if(areaid == fishZone[0] || areaid == fishZone[1])
	{
		PlayerTextDrawShow(playerid, NguDanText[playerid][0]);
		PlayerTextDrawShow(playerid, NguDanText[playerid][1]);
		PlayerTextDrawShow(playerid, NguDanText[playerid][2]);
		//GameTextForPlayer(playerid, "Ban da vao vung danh ca", 5000, 3);
...
public OnPlayerLeaveDynamicArea(playerid, areaid)
{
	if(areaid == fishZone[0] || areaid == fishZone[1])
	{
		PlayerTextDrawHide(playerid, NguDanText[playerid][0]);
		PlayerTextDrawHide(playerid, NguDanText[playerid][1]);
		PlayerTextDrawHide(playerid, NguDanText[playerid][2]);
		PlayerTextDrawHide(playerid, NguDanText[playerid][3]);
		//GameTextForPlayer(playerid, "Ban da ra vung danh ca", 5000, 3);
...
{	
	/*if(GetPlayerState(playerid) == PLAYER_STATE_SPECTATING && PlayerInfo[playerid][pNation] != 0 && gPlayerLogged{playerid} == 1 && PlayerInfo[playerid][pAdmin] == 0 && PlayerInfo[playerid][pHelper] == 0)
	{
		new string[128];
		format(string, sizeof(string), "[MAYCHU] Nguoi choi %s dang co van de, vui long kiem tra.", GetPlayerNameEx(playerid));
		ABroadCast(COLOR_LIGHTRED, string, 2);
	}*/ // 2097.48, -1806.55, 13.5
	if(IsPlayerConnected(playerid))
	{
		if(PlayerInfo[playerid][pAdmin] < 2)
		{
			if(GetPlayerWeapon(playerid) != WEAPON_PARACHUTE && GetPlayerAnimationIndex(playerid) == 958 && GetPVarInt(playerid, "EventToken") == 0)
			{
				new string[128];
				format(string, sizeof(string), "[Hack Fly] %s da bi kick khoi may chu.", GetPlayerNameEx(playerid));
				SetTimerEx("KickEx", 50, 0, "i", playerid);
				ABroadCast(COLOR_LIGHTRED,string,1);
			}
		}
	}
	if(PlayerInfo[playerid][pBuonngu] <= 20) {
		SetPlayerArmedWeapon(playerid,0);
	...
	if(GetPlayerWeapon(playerid) == 36 || GetPlayerWeapon(playerid) == 37 || GetPlayerWeapon(playerid) == 38)
    {
    	if(!IsACop(playerid) && PlayerInfo[playerid][pAdmin] == 0 && GetPVarInt(playerid, "IsInArena") == -1 && GetPVarInt( playerid, "EventToken" ) != 1)
    	{
	    	new string[128];
			ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "Banned User", " {FFFFFF}Tai khoan cua ban da bi khoa tai {FF0000}GORP: Multiplayer{FFFFFF}!\n{FF0000}Li do: {FFFFFF}Hacking (Weapons) - System\nNeu co li do bien minh ro rang, vao dien dan.", "Dong", "");
			format(string, sizeof(string), "[MAYCHU] %s da bi khoa tai khoan boi System, Reason: Hacking (Weapons)", GetPlayerNameEx(playerid));
			SendClientMessageToAllEx(COLOR_LIGHTRED, string);
			PlayerInfo[playerid][pBanned] = 1;
			StaffAccountCheck(playerid, GetPlayerIpEx(playerid));
			new ip[32];
			GetPlayerIp(playerid,ip,sizeof(ip));
	    	AddBan(playerid, playerid, "Hacking Weapons");
			MySQLBan(GetPlayerSQLId(playerid),ip,"Hacking Weapons",1, "System");
			SystemBan(playerid, GetPlayerNameEx(playerid));
	...
	if(GetPVarInt(playerid, "ClickMap") > gettime())
    {
        ...
        if(!IsPlayerInRangeOfPoint(playerid, 10.0, GetPVarFloat(playerid, "PX"), GetPVarFloat(playerid, "PY"), GetPVarFloat(playerid, "PZ")))
        {
            ...
            format(string, sizeof(string), "%s (ID %d) Dang Su DUng Hacking Teleport.", GetPlayerNameEx(playerid), playerid);
            Log("logs/telehack.log", string);
            format(string, sizeof(string), "%s (ID %d) Dang Su DUng Hacking Teleport.", GetPlayerNameEx(playerid), playerid);
            ABroadCast( COLOR_LIGHTRED, string, 2 );
            Teleport[playerid] ++;
        }
        new Float:Postl[3]; GetPlayerPos(playerid, Postl[0], Postl[1], Postl[2]);
        SetPVarFloat(playerid, "PX", Postl[0]);
        SetPVarFloat(playerid, "PY", Postl[1]);
        SetPVarFloat(playerid, "PZ", Postl[2]);
    }
    if(Teleport[playerid] > 4)
    {
        SetPlayerHealth(playerid, 0.0);
        SendClientMessage(playerid, COLOR_YELLOW, "Ban da bi kich vi hacking (TELEPORT)");
        SetTimerEx("KickEx", 1000, 0, "i", playerid);
    }
	if(GetPVarInt(playerid, "KhuVucThaLuoi") == 1)
	{
		new str[128];
		format(str, sizeof(str),"So ca hien tai:~y~%d",PlayerInfo[playerid][pKGFish]);
		PlayerTextDrawSetString(playerid, NguDanText[playerid][3],str);
		PlayerTextDrawShow(playerid, NguDanText[playerid][3]);
	}
	...
	//	SetNameTagDrawDistance(10.0);
		//SetPlayerProgressBarValue(playerid, doibung1, PlayerInfo[playerid][pHunger]);
		//ShowPlayerProgressBar(playerid, doibung1);
		//SetPlayerProgressBarValue(playerid, khatnuoc, PlayerInfo[playerid][pKhatNuoc]);
		//ShowPlayerProgressBar(playerid, khatnuoc);
		...
		PlayerTextDrawSetString(playerid, TIMESERVER[playerid],str);
		PlayerTextDrawShow(playerid, TIMESERVER[playerid]);
		format(str, sizeof(str),"%d-%d-%d",year,month,day);
		PlayerTextDrawSetString(playerid, TIMESERVER1[playerid],str);
		PlayerTextDrawShow(playerid, TIMESERVER1[playerid]);
		//format(str, 128,"%s (%d)",GetPlayerNameEx(playerid),playerid);
		//SetPlayerChatBubble(playerid, str, COLOR_WHITE, 10,190000);
	}*/
	...
	if(GetPVarInt(playerid, "Injured") == 1)
	{
		if(PlayerInfo[playerid][pChetne] == 1)
		{
	        ...
	        GameTextForPlayer(playerid, "~r~BAN DA CHET", 3000, 3);
    ...
    for(new Sz; Sz < MAX_SZ; Sz++)
    {
        if(IsPlayerInRangeOfPoint(playerid, SafeZoneInfo[Sz][szSize], SafeZoneInfo[Sz][szPosX], SafeZoneInfo[Sz][szPosY], SafeZoneInfo[Sz][szPosZ]))
        {
            if(EnterSafeZones[playerid] < 0)
            {
				if(!IsACop(playerid) || PlayerInfo[playerid][pAdmin] >= 2)
				{
					...
					//GameTextForPlayer(playerid,"~g~KHU AN TOAN",5000,6);
					SetPlayerArmedWeapon(playerid,0);
                ...
                ChatHoTro(playerid, 255, "BAN DA VAO KHU AN TOAN!",5000);
            }
            break;
        }
        if(Sz == EnterSafeZones[playerid] && EnterSafeZones[playerid] >= 0)
        {
            if(!IsPlayerInRangeOfPoint(playerid, SafeZoneInfo[Sz][szSize], SafeZoneInfo[Sz][szPosX], SafeZoneInfo[Sz][szPosY], SafeZoneInfo[Sz][szPosZ]))
            {
                ...
                //GameTextForPlayer(playerid,"~g~ROI KHOI KHU AN TOAN",5000,6);
                ChatHoTro(playerid, 255, "BAN DA ROI KHOI KHU AN TOAN!",5000);
                return 1;
            }
            break;
        }
    ...
    if(acstruct[playerid][checkmaptp] == 1 && PlayerInfo[playerid][pAdmin] < 2) //blah
	{
	    ...
	    if(dis < 5.0)
	    {
			...
			if(disd > 25.0)
			{
	            ...
	            format( srelay, sizeof( srelay ), "[MAYCHU]: %s da bi khoa tai khoan boi System | Ly do: TP Hacking", GetPlayerNameExt(playerid));
				ABroadCast( COLOR_YELLOW, srelay, 2 );
				SendClientMessage(playerid, COLOR_LIGHTRED, srelay );
				PlayerInfo[playerid][pBanned] = 3;
				new playerip[32];
				GetPlayerIp(playerid, playerip, sizeof(playerip));
				format( srelay, sizeof( srelay ), "%s (IP:%s) da bi khoa tai khoan boi System | Ly do: TP Hacking", GetPlayerNameExt(playerid), playerip);
				PlayerInfo[playerid][pBanned] = 3;
...
public OnPlayerWeaponShot(playerid, weaponid, hittype, hitid, Float:fX, Float:fY, Float:fZ)
{
	...
	if(PlayerInfo[playerid][pLevel] == 1 && PlayerInfo[playerid][pMember] == INVALID_GROUP_ID && PlayerInfo[playerid][pConnectHours] < 2) {
        if(GetPVarInt(playerid, "IsInArena") == -1 && GetPVarInt( playerid, "EventToken" ) == 0) {
			if(weaponid != 0) {
	            new String[128];
		        format( String, sizeof( String ), "[GORP-Anticheats] Nguoi choi %s , da bi banned vinh vien do su dung hacking (Kill All).", GetPlayerNameEx(playerid));
				ABroadCast( COLOR_LIGHTRED, String, 2 );
				...
				new playerip[32];
				GetPlayerIp(playerid, playerip, sizeof(playerip));
				format( String, sizeof( String ), "[MAYCHU] %s (IP:%s) da bi khoa tai khoan, ly do: Hack Kill All", GetPlayerNameEx(playerid), playerip);
				PlayerInfo[playerid][pBanned] = 3;
				Log("logs/ban.log", String);
				...
				MySQLBan(GetPlayerSQLId(playerid), playerip, "Hack KillAll", 1,"phuocngu");
				SetTimerEx("KickEx", 1000, 0, "i", playerid);
				TotalAutoBan++;
			}
		}
...
public OnPlayerClickTextDraw(playerid, Text:clickedid)
{
    	...
    	if(clickedid == Buyskin_TD[1])
	{
		ShowPlayerDialog(playerid, DIALOG_MUASKIN, DIALOG_STYLE_MSGBOX, "Mua trang phuc", "{FFFFFF}Ban co dong y mua trang phuc nay\nChi phi: {00FFFF}5,000 Credits", "Dong Y", "Huy bo");
		SetPVarInt(playerid, "ChonSkin", 1);
	}
	else if(clickedid == Buyskin_TD[2])
	{
		ShowPlayerDialog(playerid, DIALOG_MUASKIN, DIALOG_STYLE_MSGBOX, "Mua trang phuc", "{FFFFFF}Ban co dong y mua trang phuc nay\nChi phi: {00FFFF}5,000 Credits", "Dong Y", "Huy bo");
		SetPVarInt(playerid, "ChonSkin", 2);
	}
	else if(clickedid == Buyskin_TD[3])
	{
		ShowPlayerDialog(playerid, DIALOG_MUASKIN, DIALOG_STYLE_MSGBOX, "Mua trang phuc", "{FFFFFF}Ban co dong y mua trang phuc nay\nChi phi: {00FFFF}5,000 Credits", "Dong Y", "Huy bo");
		SetPVarInt(playerid, "ChonSkin", 3);
	}
	else if(clickedid == Buyskin_TD[4])
	{
		ShowPlayerDialog(playerid, DIALOG_MUASKIN, DIALOG_STYLE_MSGBOX, "Mua trang phuc", "{FFFFFF}Ban co dong y mua trang phuc nay\nChi phi: {00FFFF}5,000 Credits", "Dong Y", "Huy bo");
		SetPVarInt(playerid, "ChonSkin", 4);
	}
	else if(clickedid == Buyskin_TD[5])
	{
		ShowPlayerDialog(playerid, DIALOG_MUASKIN, DIALOG_STYLE_MSGBOX, "Mua trang phuc", "{FFFFFF}Ban co dong y mua trang phuc nay\nChi phi: {00FFFF}5,000 Credits", "Dong Y", "Huy bo");
		SetPVarInt(playerid, "ChonSkin", 5);
	}
	else if(clickedid == Buyskin_TD[6])
	{
		ShowPlayerDialog(playerid, DIALOG_MUASKIN, DIALOG_STYLE_MSGBOX, "Mua trang phuc", "{FFFFFF}Ban co dong y mua trang phuc nay\nChi phi: {00FFFF}5,000 Credits", "Dong Y", "Huy bo");
		SetPVarInt(playerid, "ChonSkin", 6);
	}
	else if(clickedid == Buyskin_TD[7])
	{
		ShowPlayerDialog(playerid, DIALOG_MUASKIN, DIALOG_STYLE_MSGBOX, "Mua trang phuc", "{FFFFFF}Ban co dong y mua trang phuc nay\nChi phi: {00FFFF}5,000 Credits", "Dong Y", "Huy bo");
		SetPVarInt(playerid, "ChonSkin", 7);
	}
	else if(clickedid == Buyskin_TD[8])
	{
		ShowPlayerDialog(playerid, DIALOG_MUASKIN, DIALOG_STYLE_MSGBOX, "Mua trang phuc", "{FFFFFF}Ban co dong y mua trang phuc nay\nChi phi: {00FFFF}5,000 Credits", "Dong Y", "Huy bo");
		SetPVarInt(playerid, "ChonSkin", 8);
	}
	else if(clickedid == Buyskin_TD[9])
	{
		ShowPlayerDialog(playerid, DIALOG_MUASKIN, DIALOG_STYLE_MSGBOX, "Mua trang phuc", "{FFFFFF}Ban co dong y mua trang phuc nay\nChi phi: {00FFFF}5,000 Credits", "Dong Y", "Huy bo");
		SetPVarInt(playerid, "ChonSkin", 9);
	}
	else if(clickedid == Buyskin_TD[10])
	{
		ShowPlayerDialog(playerid, DIALOG_MUASKIN, DIALOG_STYLE_MSGBOX, "Mua trang phuc", "{FFFFFF}Ban co dong y mua trang phuc nay\nChi phi: {00FFFF}5,000 Credits", "Dong Y", "Huy bo");
		SetPVarInt(playerid, "ChonSkin", 10);
	}
	else if(clickedid == Buyskin_TD[34])
	{
		ShowPlayerDialog(playerid, DIALOG_MUASKIN, DIALOG_STYLE_MSGBOX, "Mua trang phuc", "{FFFFFF}Ban co dong y mua trang phuc nay\nChi phi: {00FFFF}5,000 Credits", "Dong Y", "Huy bo");
		SetPVarInt(playerid, "ChonSkin", 11);
	}
	else if(clickedid == Buyskin_TD[35])
	{
		ShowPlayerDialog(playerid, DIALOG_MUASKIN, DIALOG_STYLE_MSGBOX, "Mua trang phuc", "{FFFFFF}Ban co dong y mua trang phuc nay\nChi phi: {00FFFF}5,000 Credits", "Dong Y", "Huy bo");
		SetPVarInt(playerid, "ChonSkin", 12);
	}
	else if(clickedid == Buyskin_TD[36])
	{
		ShowPlayerDialog(playerid, DIALOG_MUASKIN, DIALOG_STYLE_MSGBOX, "Mua trang phuc", "{FFFFFF}Ban co dong y mua trang phuc nay\nChi phi: {00FFFF}5,000 Credits", "Dong Y", "Huy bo");
		SetPVarInt(playerid, "ChonSkin", 13);
	}
	else if(clickedid == Buyskin_TD[37])
	{
		ShowPlayerDialog(playerid, DIALOG_MUASKIN, DIALOG_STYLE_MSGBOX, "Mua trang phuc", "{FFFFFF}Ban co dong y mua trang phuc nay\nChi phi: {00FFFF}5,000 Credits", "Dong Y", "Huy bo");
		SetPVarInt(playerid, "ChonSkin", 14);
	}
	else if(clickedid == Buyskin_TD[38])
	{
		ShowPlayerDialog(playerid, DIALOG_MUASKIN, DIALOG_STYLE_MSGBOX, "Mua trang phuc", "{FFFFFF}Ban co dong y mua trang phuc nay\nChi phi: {00FFFF}5,000 Credits", "Dong Y", "Huy bo");
		SetPVarInt(playerid, "ChonSkin", 15);
	}
	else if(clickedid == Buyskin_TD[39])
	{
		ShowPlayerDialog(playerid, DIALOG_MUASKIN, DIALOG_STYLE_MSGBOX, "Mua trang phuc", "{FFFFFF}Ban co dong y mua trang phuc nay\nChi phi: {00FFFF}5,000 Credits", "Dong Y", "Huy bo");
		SetPVarInt(playerid, "ChonSkin", 16);
	}
	else if(clickedid == Buyskin_TD[40])
	{
		ShowPlayerDialog(playerid, DIALOG_MUASKIN, DIALOG_STYLE_MSGBOX, "Mua trang phuc", "{FFFFFF}Ban co dong y mua trang phuc nay\nChi phi: {00FFFF}5,000 Credits", "Dong Y", "Huy bo");
		SetPVarInt(playerid, "ChonSkin", 17);
	}
	else if(clickedid == Buyskin_TD[41])
	{
		ShowPlayerDialog(playerid, DIALOG_MUASKIN, DIALOG_STYLE_MSGBOX, "Mua trang phuc", "{FFFFFF}Ban co dong y mua trang phuc nay\nChi phi: {00FFFF}5,000 Credits", "Dong Y", "Huy bo");
		SetPVarInt(playerid, "ChonSkin", 18);
	}
	else if(clickedid == Buyskin_TD[42])
	{
		ShowPlayerDialog(playerid, DIALOG_MUASKIN, DIALOG_STYLE_MSGBOX, "Mua trang phuc", "{FFFFFF}Ban co dong y mua trang phuc nay\nChi phi: {00FFFF}5,000 Credits", "Dong Y", "Huy bo");
		SetPVarInt(playerid, "ChonSkin", 19);
	}
	else if(clickedid == Buyskin_TD[43])
	{
		ShowPlayerDialog(playerid, DIALOG_MUASKIN, DIALOG_STYLE_MSGBOX, "Mua trang phuc", "{FFFFFF}Ban co dong y mua trang phuc nay\nChi phi: {00FFFF}5,000 Credits", "Dong Y", "Huy bo");
		SetPVarInt(playerid, "ChonSkin", 20);
	}
	else if(clickedid == Buyskin_TD[22])
	{
		ShowPlayerDialog(playerid, DIALOG_MYSKIN, DIALOG_STYLE_MSGBOX, "Trang phuc dac biet", "{FFFFFF}Ban co muon su dung trang phuc nay?", "Su dung", "Huy bo");
		SetPVarInt(playerid, "MySkin", 1);
	}
	else if(clickedid == Buyskin_TD[23])
	{
		ShowPlayerDialog(playerid, DIALOG_MYSKIN, DIALOG_STYLE_MSGBOX, "Trang phuc dac biet", "{FFFFFF}Ban co muon su dung trang phuc nay?", "Su dung", "Huy bo");
		SetPVarInt(playerid, "MySkin", 2);
	}
	else if(clickedid == Buyskin_TD[24])
	{
		ShowPlayerDialog(playerid, DIALOG_MYSKIN, DIALOG_STYLE_MSGBOX, "Trang phuc dac biet", "{FFFFFF}Ban co muon su dung trang phuc nay?", "Su dung", "Huy bo");
		SetPVarInt(playerid, "MySkin", 3);
	}
	else if(clickedid == Buyskin_TD[25])
	{
		ShowPlayerDialog(playerid, DIALOG_MYSKIN, DIALOG_STYLE_MSGBOX, "Trang phuc dac biet", "{FFFFFF}Ban co muon su dung trang phuc nay?", "Su dung", "Huy bo");
		SetPVarInt(playerid, "MySkin", 4);
	}
	else if(clickedid == Buyskin_TD[26])
	{
		ShowPlayerDialog(playerid, DIALOG_MYSKIN, DIALOG_STYLE_MSGBOX, "Trang phuc dac biet", "{FFFFFF}Ban co muon su dung trang phuc nay?", "Su dung", "Huy bo");
		SetPVarInt(playerid, "MySkin", 5);
	}
	else if(clickedid == Buyskin_TD[27])
	{
		ShowPlayerDialog(playerid, DIALOG_MYSKIN, DIALOG_STYLE_MSGBOX, "Trang phuc dac biet", "{FFFFFF}Ban co muon su dung trang phuc nay?", "Su dung", "Huy bo");
		SetPVarInt(playerid, "MySkin", 6);
	}
	else if(clickedid == Buyskin_TD[28])
	{
		ShowPlayerDialog(playerid, DIALOG_MYSKIN, DIALOG_STYLE_MSGBOX, "Trang phuc dac biet", "{FFFFFF}Ban co muon su dung trang phuc nay?", "Su dung", "Huy bo");
		SetPVarInt(playerid, "MySkin", 7);
	}
	else if(clickedid == Buyskin_TD[29])
	{
		ShowPlayerDialog(playerid, DIALOG_MYSKIN, DIALOG_STYLE_MSGBOX, "Trang phuc dac biet", "{FFFFFF}Ban co muon su dung trang phuc nay?", "Su dung", "Huy bo");
		SetPVarInt(playerid, "MySkin", 8);
	}
	else if(clickedid == Buyskin_TD[30])
	{
		ShowPlayerDialog(playerid, DIALOG_MYSKIN, DIALOG_STYLE_MSGBOX, "Trang phuc dac biet", "{FFFFFF}Ban co muon su dung trang phuc nay?", "Su dung", "Huy bo");
		SetPVarInt(playerid, "MySkin", 9);
	}
	else if(clickedid == Buyskin_TD[31])
	{
		ShowPlayerDialog(playerid, DIALOG_MYSKIN, DIALOG_STYLE_MSGBOX, "Trang phuc dac biet", "{FFFFFF}Ban co muon su dung trang phuc nay?", "Su dung", "Huy bo");
		SetPVarInt(playerid, "MySkin", 10);
	}
	else if(clickedid == Buyskin_TD[54])
	{
		ShowPlayerDialog(playerid, DIALOG_MYSKIN, DIALOG_STYLE_MSGBOX, "Trang phuc dac biet", "{FFFFFF}Ban co muon su dung trang phuc nay?", "Su dung", "Huy bo");
		SetPVarInt(playerid, "MySkin", 11);
	}
	else if(clickedid == Buyskin_TD[55])
	{
		ShowPlayerDialog(playerid, DIALOG_MYSKIN, DIALOG_STYLE_MSGBOX, "Trang phuc dac biet", "{FFFFFF}Ban co muon su dung trang phuc nay?", "Su dung", "Huy bo");
		SetPVarInt(playerid, "MySkin", 12);
	}
	else if(clickedid == Buyskin_TD[56])
	{
		ShowPlayerDialog(playerid, DIALOG_MYSKIN, DIALOG_STYLE_MSGBOX, "Trang phuc dac biet", "{FFFFFF}Ban co muon su dung trang phuc nay?", "Su dung", "Huy bo");
		SetPVarInt(playerid, "MySkin", 13);
	}
	else if(clickedid == Buyskin_TD[57])
	{
		ShowPlayerDialog(playerid, DIALOG_MYSKIN, DIALOG_STYLE_MSGBOX, "Trang phuc dac biet", "{FFFFFF}Ban co muon su dung trang phuc nay?", "Su dung", "Huy bo");
		SetPVarInt(playerid, "MySkin", 14);
	}
	else if(clickedid == Buyskin_TD[58])
	{
		ShowPlayerDialog(playerid, DIALOG_MYSKIN, DIALOG_STYLE_MSGBOX, "Trang phuc dac biet", "{FFFFFF}Ban co muon su dung trang phuc nay?", "Su dung", "Huy bo");
		SetPVarInt(playerid, "MySkin", 15);
	}
	else if(clickedid == Buyskin_TD[59])
	{
		ShowPlayerDialog(playerid, DIALOG_MYSKIN, DIALOG_STYLE_MSGBOX, "Trang phuc dac biet", "{FFFFFF}Ban co muon su dung trang phuc nay?", "Su dung", "Huy bo");
		SetPVarInt(playerid, "MySkin", 16);
	}
	else if(clickedid == Buyskin_TD[60])
	{
		ShowPlayerDialog(playerid, DIALOG_MYSKIN, DIALOG_STYLE_MSGBOX, "Trang phuc dac biet", "{FFFFFF}Ban co muon su dung trang phuc nay?", "Su dung", "Huy bo");
		SetPVarInt(playerid, "MySkin", 17);
	}
	else if(clickedid == Buyskin_TD[61])
	{
		ShowPlayerDialog(playerid, DIALOG_MYSKIN, DIALOG_STYLE_MSGBOX, "Trang phuc dac biet", "{FFFFFF}Ban co muon su dung trang phuc nay?", "Su dung", "Huy bo");
		SetPVarInt(playerid, "MySkin", 18);
	}
	else if(clickedid == Buyskin_TD[62])
	{
		ShowPlayerDialog(playerid, DIALOG_MYSKIN, DIALOG_STYLE_MSGBOX, "Trang phuc dac biet", "{FFFFFF}Ban co muon su dung trang phuc nay?", "Su dung", "Huy bo");
		SetPVarInt(playerid, "MySkin", 19);
	}
	else if(clickedid == Buyskin_TD[63])
	{
		ShowPlayerDialog(playerid, DIALOG_MYSKIN, DIALOG_STYLE_MSGBOX, "Trang phuc dac biet", "{FFFFFF}Ban co muon su dung trang phuc nay?", "Su dung", "Huy bo");
		SetPVarInt(playerid, "MySkin", 20);
	}
	else if(clickedid == Buyskin_TD[32]) // trang 1
	{
		if(GetPVarInt(playerid, "Skinshop") == 2)
		{
 	      for(new i = 35; i < 54; i++)
	      {
           TextDrawHideForPlayer(playerid, Buyskin_TD[i]);
	      }
 	      for(new i = 1; i < 22; i++)
	      {
		...
		if(GetPVarInt(playerid, "Selecskin") == 2)
		{
 	    for(new i = 1; i < 64; i++)
	     {
           TextDrawHideForPlayer(playerid, Buyskin_TD[i]);
	...
	else if(clickedid == Buyskin_TD[33]) // trang 2
	{
		if(GetPVarInt(playerid, "Skinshop") == 1)
		{
	      ...
	      {
            TextDrawShowForPlayer(playerid, Buyskin_TD[i]);
            TextDrawShowForPlayer(playerid, Buyskin_TD[21]);
	      }
	      SetPVarInt(playerid, "Skinshop", 2);
	    }
		if(GetPVarInt(playerid, "Selecskin") == 1) // trang 2 my skin
		{
 	      for(new i = 1; i < 44; i++)
	      {
            TextDrawHideForPlayer(playerid, Buyskin_TD[i]);
	      ...
	      SetPVarInt(playerid, "Selecskin", 2);
	    }
	}
/*
	else if(clickedid == Buyskin_TD[33]) // trang 2
	{
		if(GetPVarInt(playerid, "Myskin") == 1)
		{
 	      for(new i = 1; i < 44; i++)
	      {
            TextDrawHideForPlayer(playerid, Buyskin_TD[i]);
	...
	else if(clickedid == Buyskin_TD[33]) // trang 1
	{
		if(GetPVarInt(playerid, "Myskin") == 1)
		{
 	    for(new i = 1; i < 64; i++)
	...
	else if(clickedid == Text:INVALID_TEXT_DRAW)
	{
	     ...
	     {
           TextDrawHideForPlayer(playerid, Buyskin_TD[i]);
           DeletePVar(playerid, "Skinshop");
           DeletePVar(playerid, "Myskin");
	     }
	}
	if(clickedid == Buyskin_TD[21])
	{
 	    for(new i = 0; i < 64; i++)
	     {
           TextDrawHideForPlayer(playerid, Buyskin_TD[i]);
...
{
//    Inv_OnClick(playerid, playertextid);
	BeKhoa_OnPlayerClickPTD(playerid, playertextid);
	
    if(playertextid == RegisterTD[playerid][17]) //quatraii
    {
        PlayerTextDrawDestroy(playerid,RegisterTD[playerid][19]);
        Skinzz[playerid]--;
        if(Skinzz[playerid] < 4) Skinzz[playerid] = 11;
		RegisterTD[playerid][19] = CreatePlayerTextDraw(playerid, 355.000000, 201.000000, "Preview_Model");
		PlayerTextDrawFont(playerid, RegisterTD[playerid][19], 5);
		PlayerTextDrawLetterSize(playerid, RegisterTD[playerid][19], 0.600000, 2.000000);
		PlayerTextDrawTextSize(playerid, RegisterTD[playerid][19], 81.500000, 80.500000);
		...
		PlayerTextDrawBackgroundColor(playerid, RegisterTD[playerid][19], 0);
		PlayerTextDrawBoxColor(playerid, RegisterTD[playerid][19], 48);
		PlayerTextDrawUseBox(playerid, RegisterTD[playerid][19], 0);
		PlayerTextDrawSetProportional(playerid, RegisterTD[playerid][19], 1);
		PlayerTextDrawSetSelectable(playerid, RegisterTD[playerid][19], 0);
		PlayerTextDrawSetPreviewModel(playerid, RegisterTD[playerid][19], Skinzz[playerid]);
		PlayerTextDrawSetPreviewRot(playerid, RegisterTD[playerid][19], -10.000000, 0.000000, -5.000000, 0.959999);
		PlayerTextDrawSetPreviewVehCol(playerid, RegisterTD[playerid][19], 0, 0);
		await task_ms(100);
		PlayerTextDrawShow(playerid,RegisterTD[playerid][19]);

	}
    if(playertextid == RegisterTD[playerid][18]) //quaphai
    {
        PlayerTextDrawDestroy(playerid,RegisterTD[playerid][19]);
        Skinzz[playerid]++;
        if(Skinzz[playerid] > 11) Skinzz[playerid] = 4;
		RegisterTD[playerid][19] = CreatePlayerTextDraw(playerid, 355.000000, 201.000000, "Preview_Model");
		PlayerTextDrawFont(playerid, RegisterTD[playerid][19], 5);
		PlayerTextDrawLetterSize(playerid, RegisterTD[playerid][19], 0.600000, 2.000000);
		PlayerTextDrawTextSize(playerid, RegisterTD[playerid][19], 81.500000, 80.500000);
    ...
    {
		ShowPlayerDialog(playerid, DIALOG_DANGNHAP,DIALOG_STYLE_INPUT,"Password","Nhap mat khau cua ban.","Nhap","Huy");
		CancelSelectTextDraw(playerid);
    }
    if(playertextid == RegisterTD[playerid][12])
    {
		ShowPlayerDialog(playerid, DIALOG_DANGNHAPEMAIL,DIALOG_STYLE_INPUT,"EMAIL","Nhap EMAIL cua ban.","Nhap","Huy");
		CancelSelectTextDraw(playerid);
    }
    if(playertextid == RegisterTD[playerid][33])
    {
		new lastdate = 31, stringdiag[410];
		for(new x = 1; x < lastdate; x++)
		{
			format(stringdiag, sizeof(stringdiag), "%s%d\n", stringdiag, x);
		}
		ShowPlayerDialog(playerid, DIALOG_NGAYSINH, DIALOG_STYLE_LIST, "{FF0000}Nhan vat cua ban duoc sinh ra ngay nao?", stringdiag, "Chon", "Dong");
		CancelSelectTextDraw(playerid);
    }
    if(playertextid == RegisterTD[playerid][20])
    {
        new str[64],email[64],query[333],string[333],stringz[323];
        GetPVarString(playerid,"PassAuth",str,64);
        GetPVarString(playerid,"EmailAuth",email,64);
        new ngaysinh = GetPVarInt(playerid, "ngaysinh"),thangsinh = GetPVarInt(playerid, "thangsinh"),namsinh = GetPVarInt(playerid, "namsinh");
        if(!isnull(str) && !isnull(email) && ngaysinh && thangsinh && namsinh)
        {
			g_mysql_CreateAccount(playerid, str);
			Hidedangky(playerid);
			// await task_ms(2000);
			format(query, sizeof(query), "UPDATE `accounts` SET `Email`='%s' WHERE `id`=%d", email, GetPlayerSQLId(playerid));
			mysql_function_query(MainPipeline, query, false, "OnQueryFinish", "i", SENDDATA_THREAD);

			format(stringz, 323, "%d-%02d-%02d", namsinh,thangsinh,ngaysinh);

		    format(string, sizeof(string), "Nobody");
			strmid(PlayerInfo[playerid][pReferredBy], string, 0, strlen(string), MAX_PLAYER_NAME);
			strmid(PlayerInfo[playerid][pBirthDate], stringz, 0, strlen(stringz), 11);
...
//			UpdateHungerBar(playerid);
			...
			DeletePVar(playerid,"ngaysinh");
			DeletePVar(playerid,"thangsinh");
			DeletePVar(playerid,"namsinh");
			DeletePVar(playerid,"PassAuth");
			DeletePVar(playerid,"EmailAuth");
		}
		else
		{
			ShowPlayerDialog(playerid,32142,DIALOG_STYLE_MSGBOX,"{FF0000}ERROR","{FFFFFF}Co loi xay ra\nVui long dien day du thong tin!","Dong","");
		}
    }
////////////////////////////////////////////////////////////////////////////////////////////////
	if(playertextid == ChonXe_TD[playerid][0])
	{
		if(GetPVarInt(playerid,"buycars") == 1)
		{
		    ...
		    testcarpos1[playerid] = 1557.1479;
		    testcarpos2[playerid] = -1253.4930;
		    testcarpos3[playerid] = 17.5355;
     		IsPlayerEntering{playerid} = true;
            new car = CreateVehicle(modelveh, 1575.3860,-1241.8801,17.2406,176.4339, 126, 126, -1);
            PutPlayerInVehicle(playerid, car, 0);
			VehicleFuel[car] = 100.0;
			Vehicle_ResetData(car);
			SendClientMessageEx(playerid, COLOR_YELLOW, "Ban co 3 phut de test.");
			SendClientMessageEx(playerid, COLOR_YELLOW, "Neu ban khong con muon test nua, ban co the ket thuc bang cach ra khoi xe.");
		...
		SendClientMessageEx(playerid,-1,"nenene");
	}
	if(playertextid == ChonXe_TD[playerid][1])
	{
	    for(new i=0;i<2;i++) PlayerTextDrawHide(playerid,ChonXe_TD[playerid][i]);
		for(new i=0;i<20;i++) PlayerTextDrawHide(playerid,BanXe_TD[playerid][i]);
		new string[128];
		...
		CancelSelectTextDraw(playerid);
		format(string, sizeof(string), "Ban co muon mua xe %s.",GetVehicleName(newcar));
...
public OnPlayerModelSelection(playerid, response, listid, modelid)
{
	...
	if(listid == ShopxeAudi)
	{
		if(response)
		{
			ShopxeAudi = LoadModelSelectionMenu("ShopxeAudi.txt");
			xemua = CreateVehicle(modelid, 1560.2932,-1242.2970,17.9151,88.7460, 1, 1, -1);
			vehicle_lock_doors(xemua);
			PutPlayerInVehicle(playerid,xemua,0);
			PlayerTextDrawSetPreviewModel(playerid, BanXe_TD[playerid][5], modelid);
			PlayerTextDrawSetPreviewModel(playerid, BanXe_TD[playerid][11], modelid);
			PlayerTextDrawSetPreviewModel(playerid, BanXe_TD[playerid][12], modelid);
			SelectTextDraw(playerid,-1);
			if(modelid == 422)
			{
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][6],"Vehicle : Bobcat");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][7],"Speed Limit : 78km/h");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][8],"Health : 1000 HP");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][9],"Slot : 2");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][10],"TypeCar : Truck");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][16],"20.000 $");
				SetPVarInt(playerid,"GiaxeAudi",20000);
				for(new i=0;i<2;i++) PlayerTextDrawShow(playerid,ChonXe_TD[playerid][i]);
				for(new i=0;i<20;i++) PlayerTextDrawShow(playerid,BanXe_TD[playerid][i]);
			}
			if(modelid == 401)
			{
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][6],"Vehicle : Bravura");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][7],"Speed Limit : 82km/h");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][8],"Health : 1000 HP");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][9],"Slot : 2");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][10],"TypeCar : Normal");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][16],"30.000 $");
				for(new i=0;i<2;i++) PlayerTextDrawShow(playerid,ChonXe_TD[playerid][i]);
				for(new i=0;i<20;i++) PlayerTextDrawShow(playerid,BanXe_TD[playerid][i]);
				SetPVarInt(playerid,"GiaxeAudi",30000);
			}
			if(modelid == 462)
			{
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][6],"Vehicle : Faggio");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][7],"Speed Limit : 60km/h");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][8],"Health : 1000 HP");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][9],"Slot : 2");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][10],"TypeCar : MotorBike");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][16],"15.000 $");
				for(new i=0;i<2;i++) PlayerTextDrawShow(playerid,ChonXe_TD[playerid][i]);
				for(new i=0;i<20;i++) PlayerTextDrawShow(playerid,BanXe_TD[playerid][i]);
				SetPVarInt(playerid,"GiaxeAudi",15000);
			}
			if(modelid == 405)
			{
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][6],"Vehicle : Sentinel");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][7],"Speed Limit : 91km/h");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][8],"Health : 1000 HP");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][9],"Slot : 4");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][10],"TypeCar : Normal");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][16],"40.000 $");
				for(new i=0;i<2;i++) PlayerTextDrawShow(playerid,ChonXe_TD[playerid][i]);
				for(new i=0;i<20;i++) PlayerTextDrawShow(playerid,BanXe_TD[playerid][i]);
				SetPVarInt(playerid,"GiaxeAudi",40000);
			}
			if(modelid == 418)
			{
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][6],"Vehicle : MoonBeam");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][7],"Speed Limit : 64km/h");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][8],"Health : 1000 HP");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][9],"Slot : 4");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][10],"TypeCar : Truck");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][16],"25.000 $");
				for(new i=0;i<2;i++) PlayerTextDrawShow(playerid,ChonXe_TD[playerid][i]);
				for(new i=0;i<20;i++) PlayerTextDrawShow(playerid,BanXe_TD[playerid][i]);
				SetPVarInt(playerid,"GiaxeAudi",25000);
			}
			if(modelid == 445)
			{
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][6],"Vehicle : Admiral");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][7],"Speed Limit : 91km/h");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][8],"Health : 1000 HP");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][9],"Slot : 4");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][10],"TypeCar : Normal");
			...
			if(modelid == 436)
			{
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][6],"Vehicle : Previon");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][7],"Speed Limit : 83km/h");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][8],"Health : 1000 HP");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][9],"Slot : 2");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][10],"TypeCar : Normal");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][16],"30.000 $");
			...
			if(modelid == 461)
			{
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][6],"Vehicle : PCJ-600");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][7],"Speed Limit : 88km/h");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][8],"Health : 1000 HP");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][9],"Slot : 2");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][10],"TypeCar : MotorBike");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][16],"45.000 $");
				for(new i=0;i<2;i++) PlayerTextDrawShow(playerid,ChonXe_TD[playerid][i]);
				for(new i=0;i<20;i++) PlayerTextDrawShow(playerid,BanXe_TD[playerid][i]);
				SetPVarInt(playerid,"GiaxeAudi",45000);
			}
			if(modelid == 439)
			{
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][6],"Vehicle : Stallion");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][7],"Speed Limit : 94km/h");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][8],"Health : 1000 HP");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][9],"Slot : 2");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][10],"TypeCar : Convertibles");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][16],"40.000 $");
				for(new i=0;i<2;i++) PlayerTextDrawShow(playerid,ChonXe_TD[playerid][i]);
				for(new i=0;i<20;i++) PlayerTextDrawShow(playerid,BanXe_TD[playerid][i]);
				SetPVarInt(playerid,"GiaxeAudi",40000);
			...
			if(modelid == 410)
			{
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][6],"Vehicle : Manana");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][7],"Speed Limit : 72km/h");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][8],"Health : 1000 HP");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][9],"Slot : 2");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][10],"TypeCar : Normal");
				PlayerTextDrawSetString(playerid, BanXe_TD[playerid][16],"20.000 $");
			...
			ChatHoTro(playerid,255,"Ban da roi cua hang audi",5000);
			dangmuaxe = 0;
		}
 }
	...
	if(listid == SkinList)
	{
		if(response)
		{
			if(PlayerInfo[playerid][pDonateRank] >= 2)
			{
				...
				return SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da thay doi skin.");
			}
...
{
	/*if(PlayerInfo[playerid][pConnectHours] < 2 && PlayerInfo[playerid][pWeapons] == 0 && GetPVarInt(playerid, "IsInArena") == -1 && GetPVarInt( playerid, "EventToken" ) != 1)
	{
		{
			if(weaponid == 9  || weaponid == 16  ||  weaponid == 17  ||  weaponid == 18 || weaponid == 22 || weaponid == 23 || weaponid == 24 || weaponid == 25 || weaponid == 26 || weaponid == 27 || weaponid == 28 || weaponid == 29 || weaponid == 30 || weaponid == 31 || weaponid == 32 || weaponid == 33 || weaponid == 34 || weaponid == 35 || weaponid == 36 || weaponid == 37 || weaponid == 38 || weaponid == 39 || weaponid == 40 || weaponid == 41 || weaponid == 42)
			{
                	new string[128];
                	format(string, sizeof(string), "[GORP-Anticheats]: %s(ID: %d) Co The Dang Hack Weapon (%s).", GetPlayerNameEx(playerid), playerid, GetWeaponNameEx(weaponid));
                	ABroadCast(COLOR_LIGHTRED, string, 2);
	...
	if(issuerid != INVALID_PLAYER_ID)
	{
		...
		{	
			if(vehiclecp[issuerid] == 5)
			{
				SetVehiclePos(issuerid, 0.0,0.0,0.0);
				await task_ms(100);
				SetPlayerPos(issuerid, 0.0,0.0,0.0);
				SendClientMessage(issuerid, -1, "Noi roi khong chiu nghe ra day choi voi khi di");
			}
			vehiclecp[issuerid] ++;
			SendClientMessage(issuerid, -1, "{ec5f00}[Warinng]{d0d889} khong su dung phuong tien tan cong nguoi khac neu tai pham 5 lan se bi tele");
			await task_ms(10000);
			vehiclecp[issuerid] = 0;
		}*/
    			...
    			if(weaponid != 54 && weaponid != 4)
	    		{
					if(GetPVarInt(playerid, "CleoWarning") == 2)
					{
						...
						if(PlayerInfo[playerid][pLevel] > 3) {
						    format(string, sizeof(string), "[MAYCHU] %s da bi vao tu boi Admin | ly do: Troi buoc!", GetPlayerNameEx(playerid), GetPlayerNameEx(playerid));
							SendClientMessageToAllEx(COLOR_LIGHTRED, string);
						}
						else {
						    format(string, sizeof(string), "[MAYCHU]: %s da bi vao tu boi Admin | ly do: Troi buoc", GetPlayerNameEx(playerid), GetPlayerNameEx(playerid));
...
public OnPlayerGiveDamage(playerid, damagedid, Float:amount, weaponid, bodypart)
{
	...
	if(PlayerInfo[playerid][pConnectHours] < 2 && PlayerInfo[playerid][pWeapons] == 0 && GetPVarInt(playerid, "IsInArena") == -1 && GetPVarInt( playerid, "EventToken" ) != 1)
	{
		if(weaponid == 9 || weaponid >= 16  && weaponid <= 42)
		{
			...
			SendClientMessageEx(playerid, COLOR_LIGHTRED,"Vi ban la nguoi choi moi nen khong the gay sat thuong.");
			return 0;
		}
	}
	if(AntiSK[damagedid] == 1)
	{
		SendClientMessageEx(playerid, COLOR_LIGHTRED, "[ANTI SK]{FFFFFF} Nguoi choi dang trong qua trinh loading door/house, khong the tan cong.");
		SetPlayerHealth(damagedid, 100);
		return 1;
	}
	...
	{
		new string[128];
		SendClientMessageEx(playerid, COLOR_LIGHTRED, "[ANTI KILLALL]{FFFFFF} Ban khong the gay sat thuong cho nguoi choi o khoang cach xa 5m.");
		SetPlayerHealth(damagedid, 100);
		format(string, sizeof(string), "[GORP-Anticheats] %s (ID %d) dang co gay sat thuong len nguoi choi tu xa.", GetPlayerNameEx(playerid), playerid);
		ABroadCast( COLOR_LIGHTRED, string, 2 );
		return 1;
	}
    ...
    if(weaponid == 0)
        format(stringdz, sizeof(stringdz), "Ban da dung {ff9900}Unarmed{FFFFFF} gay {ff9900}%.2f{FFFFFF} %s cho {ff9900}%s{FFFFFF} (%0.2fm)", amount,  GetBodyPartName(bodypart), victim, Distance);
    else
        format(stringdz, sizeof(stringdz), "Ban da dung {ff9900}%s{FFFFFF} gay {ff9900}%.2f{FFFFFF} %s cho {ff9900}%s{FFFFFF} (%0.2fm)", weaponname, amount,  GetBodyPartName(bodypart), victim, Distance, GetBodyPartName(bodypart));
    SendClientMessage(playerid, -1, stringdz);

    //___________________________________________________________________________________________________________________________________________________________________________________________________________________________________//

    new infoString[2000], weaponName[24], attackerName[MAX_PLAYER_NAME];
    GetPlayerName(playerid, attackerName, sizeof (attackerName));
    GetWeaponName(weaponid, weaponName, sizeof (weaponName));

    if(weaponid == 0)
         format(infoString, sizeof(infoString), "{ff9900}%s{FFFFFF} da su dung {ff9900}Unarmed{FFFFFF} gay cho ban {ff9900}%.2f{FFFFFF} %s (%0.2fm)", attackerName, amount,  GetBodyPartName(bodypart), Distance);
     else
        format(infoString, sizeof(infoString), "{ff9900}%s{FFFFFF} da su dung {ff9900}%s{FFFFFF} gay cho ban {ff9900}%.2f{FFFFFF} %s (%0.2fm)", attackerName, weaponName, amount,  GetBodyPartName(bodypart), Distance);
    SendClientMessageEx(damagedid, -1, infoString);
    ...
    /* if(pTazer{playerid} == 1)
	{
	    if(weaponid !=  23) {
			...
			GivePlayerValidWeapon(playerid, pTazerReplace{playerid}, 1);
		...
		if(GetPlayerState(damagedid) == PLAYER_STATE_ONFOOT && PlayerCuffed[damagedid] == 0 && PlayerInfo[playerid][pHasTazer] == 1)
		{
			...
			ApplyAnimation(damagedid,"GYMNASIUM","gym_jog_falloff",4.1,0,1,1,1,1,1);
	...
	/* if(pBean{playerid} == 1)
	{
	    if(weaponid !=  25) {
			...
			GivePlayerValidWeapon(playerid, pBeanReplace{playerid}, 30);
			format(string, sizeof(string), "%s da lay mot khau sung cao su tu sau lung", GetPlayerNameEx(playerid));
			ProxDetector(30.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
			pBean{playerid} = 0;
			return 1;
		}
		if(!ProxDetectorS(20.0, playerid, damagedid)) {
			new string[44 + (MAX_PLAYER_NAME * 2)];
			format(string, sizeof(string), "* %s da ban vao %s, ban truot", GetPlayerNameEx(playerid), GetPlayerNameEx(damagedid));
			ProxDetector(30.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
			return 1;
		}
 		if(BeanTimeout[playerid] > 0 && !GetPVarType(damagedid, "IsFrozen"))
  		{
  		    new Float:hp;
  		    GetPlayerHealth(damagedid, hp);
  		    SetPlayerHealth(damagedid, hp-amount);
		...
		if(GetPlayerState(damagedid) == PLAYER_STATE_ONFOOT && PlayerCuffed[damagedid] == 0 && PlayerInfo[playerid][pHasTazer] == 1)
		{
		    if(PlayerInfo[damagedid][pAdmin] >= 2 && PlayerInfo[damagedid][pTogReports] != 1)
			{
			    SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong the ban vao admin");
			    new Float:hp;
	  		    GetPlayerHealth(damagedid, hp);
	  		    SetPlayerHealth(damagedid, hp+amount);
				return 1;
			...
			GetPlayerHealth(damagedid, hp);
			new string[44 + (MAX_PLAYER_NAME * 2)];
			format(string, sizeof(string), "* %s da ban mot vien dan cao su vao nguoi %s .", GetPlayerNameEx(playerid), GetPlayerNameEx(damagedid));
			ProxDetector(30.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
			SendClientMessage(damagedid, COLOR_GREY,"Ban da bi nga vi bi ban");
   			SetPlayerHealth(damagedid, hp+amount);
			//TogglePlayerControllable(damagedid, 0);
			ApplyAnimation(damagedid, "SWEET", "Sweet_injuredloop",4.1,0,1,1,1,30000,0);
			PlayerPlaySound(damagedid, 1085, X, Y, Z);
			PlayerPlaySound(playerid, 1085, X, Y, Z);
			PlayerCuffed[damagedid] = 1;
			SetPVarInt(damagedid, "PlayerCuffed", 1);
			PlayerCuffedTime[damagedid] = 16;
			BeanTimeout[playerid] = 0;
			SetPVarInt(damagedid, "IsFrozen", 1);
		}
	} */
	/* -------------------------------- Tazer -------------------------------- */
	if(pTazer{playerid} == 1)
	{
		if(weaponid !=  23)
		{
			new string[44 + MAX_PLAYER_NAME];
			RemovePlayerWeapon(playerid, 23);
			GivePlayerValidWeapon(playerid, pTazerReplace{playerid}, PlayerInfo[playerid][pAmmo][GetAmmoSlot(pTazerReplace{playerid})]);
			format(string, sizeof(string), "* %s lay ra khau sung dien.", GetPlayerNameEx(playerid));
			ProxDetector(30.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
			pTazer{playerid} = 0;
			return 1;
		}
		if(!ProxDetectorS(20.0, playerid, damagedid)) {
			new string[44 + (MAX_PLAYER_NAME * 2)];
			format(string, sizeof(string), "* %s ban sung dien vao %s, nhung truot.", GetPlayerNameEx(playerid), GetPlayerNameEx(damagedid));
			ProxDetector(30.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
			return 1;
		}
		if(TazerTimeout[playerid] > 0 && !GetPVarType(damagedid, "IsFrozen"))
		{
			...
			GetPlayerHealth(damagedid, hp);
			PlayerInfo[damagedid][pHealth] = hp;
			SetPlayerHealth(damagedid, hp);
			GetPlayerArmour(damagedid, armor);
			SetPlayerArmor(damagedid, armor);
			return 1;
		}
		if(GetPVarInt(damagedid, "IsFrozen") == 1)
		{
			new Float:hp, Float:armor;
			GetPlayerHealth(damagedid, hp);
			PlayerInfo[damagedid][pHealth] = hp;
		...
		if(PlayerCuffed[damagedid] == 0 && PlayerInfo[playerid][pHasTazer] == 1)
		{
			/* if(PlayerInfo[damagedid][pAdmin] >= 2 && PlayerInfo[damagedid][pTogReports] != 1)
			{
			...
			format(string, sizeof(string), "* %s da ban sung dien lam te liet %s.", GetPlayerNameEx(playerid), GetPlayerNameEx(damagedid));
			ProxDetector(30.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
			SetPlayerChatBubble(damagedid,"(( Dang bi te liet ))",COLOR_ORANGE,100.0,10000);
			GameTextForPlayer(damagedid, "~r~Tazed", 3500, 3);
			PlayerInfo[damagedid][pHealth] = hp;
			SetPlayerHealth(damagedid, hp);
			SetPlayerArmor(damagedid, armor);
			...
			SetPVarInt(damagedid, "IsFrozen", 1);
			SetPVarInt(damagedid, "BiTeLiet", 1);
			TazerTimeout[playerid] = 6;
			ApplyAnimation(damagedid,"SWEET", "Sweet_injuredloop",4.1,1,1,1,1,1,1);
			SetTimerEx("TazerTimer",1000,false,"d",playerid);
			GameTextForPlayer(playerid, "~n~~n~~n~~n~~n~~n~~n~~n~~r~Dang nap dan... ~w~5", 1500,3);
		}
	}
	/* -------------------------------- Bean bag -------------------------------- */
	//beanbag
	if(pTazer{playerid} == 2)
	{
		if(weaponid == 25)
		{
			if(!ProxDetectorS(20.0, playerid, damagedid)) {
			...
			if(PlayerCuffed[damagedid] == 0 && PlayerInfo[playerid][pHasTazer] == 1)
			{
				...
				//
				new string[44 + (MAX_PLAYER_NAME * 2)];
				format(string, sizeof(string), "* %s lam choang doi tuong %s bang beanbag.", GetPlayerNameEx(playerid), GetPlayerNameEx(damagedid));
				ProxDetector(30.0, playerid, string, COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE,COLOR_PURPLE);
				SetPlayerChatBubble(damagedid,"(( Dang bi choang ))",COLOR_ORANGE,100.0,10000);
				GameTextForPlayer(damagedid, "~r~Tazed", 3500, 3);
				PlayerInfo[damagedid][pHealth] = hp;
				SetPlayerHealth(damagedid, hp);
				SetPlayerArmor(damagedid, armor);
	...
	for(new Sz; Sz < MAX_SZ; Sz++)
	{
		...
		{// If Player In Safe Zone
			if(PlayerInfo[playerid][pAdmin] < 1)
			{
				if(!IsACop(playerid))
				{
					SetTimerEx("LoadPlayer", 2000,0, "d",playerid); // 2s
					SendClientMessage(playerid, -1, "Han che tan cong nguoi khac nao");
					TogglePlayerControllable(playerid, 0);
				}
			}
		}
	...
	if(((GetTickCount() - pSSHealthTime[playerid][0]) > (pSSHealthTime[playerid][1] * 2)))
	{
	    if(weaponid != 54 && weaponid != 37 && weaponid != 38 && weaponid != 51 && weaponid != 53)
	    {
			...
			if((pSSArmour[playerid] > 0) && (((pSSArmour[playerid] > armour) && ((pSSArmour[playerid]-armour) > 1)) || ((pSSArmour[playerid] < armour) && ((armour-pSSArmour[playerid]) > 1))))
			{
			    ...
			    {
			        new string[128];
					format(string, sizeof(string), "[MAYCHU]: %s (ID %d) is possibly armour hacking", GetPlayerNameEx(playerid), playerid);
			...
			if((pSSHealth[playerid] > 0) && (((pSSHealth[playerid] > HP) && ((pSSHealth[playerid]-HP) > 1)) || ((pSSHealth[playerid] < HP) && ((HP-pSSHealth[playerid]) > 1))))
		 	{
		 	    ...
		 	    {
		 	        new string[128];
					format(string, sizeof(string), "[MAYCHU]: %s (ID %d) is possibly health hacking", GetPlayerNameEx(playerid), playerid);
			 		ABroadCast( COLOR_YELLOW, string, 2 );
...
public OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
	...
	{
		new str[128];
		format(str, 300, "'TRUCKER' Phuong tien thuoc so huu boi ban");
	}
	if(PizzaPlayer[playerid][Player_Vehicle_ID] == vehicleid)
	{
		new str[300];
		format(str, 300, "Day la phuong tien cua ban ma so phuong tien {990000}%d",vehicleid);
		SendClientMessageEx(playerid, COLOR_YELLOW, str);
	}
	/*if(PLayerInfo[playerid][pCarLic] = 0)
	{
		PlayAudioStreamForPlayer(playerid, "http://web.samp-vn.com/giaypheplaixe.mp3");	
	}*/
    IsPlayerEntering{playerid} = true;
	Seatbelt[playerid] = 0;
    ...
    if(GetPVarType(playerid, "Pizza") && !(IsAPizzaCar(vehicleid)) && GetPVarInt(playerid, "DenDichXuongXe") == 0)
	{
	...
	if(!ispassenger)
	{
		...
		else if(IsWeaponizedVehicle(GetVehicleModel(vehicleid)))
	    {
	        if(PlayerInfo[playerid][pLevel] < 6)
	        {
				...
				{
					new szString[128];
					format(szString, sizeof(szString), "[MAYCHU]: %s (ID: %d) da vao phuong tien hang nang (ID XE: %d) (Level: %d)", GetPlayerNameEx(playerid), playerid, vehicleid, PlayerInfo[playerid][pLevel]);
		...
		else if(IsAFixCar(vehicleid))
		{
		    if(PlayerInfo[playerid][pNhanvienxang] != 1)
		    {
			    ...
			    RemovePlayerFromVehicle(playerid);
			    defer NOPCheck(playerid);
			    SendClientMessageEx(playerid, COLOR_GRAD2, "Ban khong phai la nguoi cua cua hang");
			}
			else
			{
				SendClientMessage(playerid, -1, "Day la phuong tien cua cua hang vui long khong lam viec rieng");
			}
		}
		else if(IsaCopa(vehicleid))
		{
		    if(!IsACop(playerid))
		    {
			    new Float:slx, Float:sly, Float:slz;
			    ...
			    RemovePlayerFromVehicle(playerid);
			    defer NOPCheck(playerid);
			    SendClientMessageEx(playerid, COLOR_GREY, "Ban khong phai la nguoi cua to chuc");
			}
			else
			{
		...
		else if(IsAGiaoHang1Car(vehicleid))
		{
		    if(PlayerInfo[playerid][pJob] != 23 && PlayerInfo[playerid][pJob2] != 23)
		    {
			    ...
			    RemovePlayerFromVehicle(playerid);
			    defer NOPCheck(playerid);
			    SendClientMessageEx(playerid, COLOR_GREY, "Ban khong phai nguoi van chuyen");
			}
		}
		...
		else if(IsATruckerCar(vehicleid))
		{
		    if((PlayerInfo[playerid][pJob] == 20 || PlayerInfo[playerid][pJob2] == 20))
			{
				...
				format(string, sizeof(string), "TRUCKER: (Dang ky xe: %s %d)", GetVehicleName(vehicleid), vehicleid, truckcontentname);
				...
				{
    				//SendClientMessageEx(playerid, COLOR_YELLOW, "TRUCKER JOB: Su dung /layhang de nhan hang hoa.");
				}
...
{
	ThiBangLai_VehDamage(vehicleid, playerid);
    return 1;
}
...
public OnPlayerConnect(playerid)
{
	...
	TextDrawDestroy(Notifix[0]);
	TextDrawDestroy(Notifix[1]);
	TextDrawDestroy(Notifix[2]);
	TextDrawDestroy(Notifix[3]);
	...
	TextDrawShowForPlayer(playerid, logocustom[0]);
	TogglePlayerControllable(playerid, 0);
...
//    tddoikhat(playerid);
	...
	new string[20];
	format(string, sizeof(string), "Player_%d", playerid);
	SetPlayerNameInServerQuery(playerid, string);
...
{
	if(GetPVarInt(playerid, "Injured") == 1) GetPlayerPos(playerid, PlayerInfo[playerid][pDeathPos][0], PlayerInfo[playerid][pDeathPos][1], PlayerInfo[playerid][pDeathPos][2]);
	TrollCar[playerid] = INVALID_PLAYER_ID;
    DeletePVar(playerid, "KhongHacking");
    // DeletePVar(playerid, "NotAndroid");
	if(PlayerInfo[playerid][pTruyDuoi] ==1)
    {
            ...
            SetPlayerVirtualWorld(playerid, 0);
            format(strings, sizeof(strings), "[PRISON] Ban da bi vao tu vi AR ((/truyduoi)");
            SendClientMessageEx(playerid, COLOR_LIGHTBLUE, strings);
            format(PlayerInfo[playerid][pPrisonReason], 128, "[OOC][JAIL] AR ((/truyduoi)");
            SetPlayerColor(playerid, TEAM_APRISON_COLOR);
            new string[1280];
            format(string, sizeof(string), "[MAYCHU] %s da bi vao tu | Ly do: AR", GetPlayerNameEx(playerid));
            SendClientMessageToAllEx(COLOR_LIGHTRED, string);
    }
    if(IsACop(playerid))
    {
...
//	PlayerTextDrawHide(playerid,logo1[playerid]);
	if(IsPlayerInRangeOfPoint(playerid, 2.0, 50.0000,50.0000,50.0000))
	{
		SetPlayerPos(playerid, 2094.0977,-1805.6383,13.5503);
	}
	if(socuu[playerid] == 1)
	{
		KillTimer(socuu[playerid]);
		DeletePVar(playerid, "SOCUU");
	}
	if(TimeFixVehicles[playerid] == 1)
	{
		DeletePVar(playerid, "Fixvehicles");
		KillTimer(TimeFixVehicles[playerid]);
	}
	/*if(LoadingPlayer[playerid] == 1)
	{
		DeletePVar(playerid, "LOADTK");
		KillTimer(LoadingPlayer[playerid]);
	}*/
	if(LoadingPlayer[playerid] == 1)
	{
		KillTimer(xiutaicho[playerid]);
	}
	if(timemask[playerid] == 1)
	{
		KillTimer(timemask[playerid]);
		DeletePVar(playerid, "TIMEMASK");
	}
	PlayerInfo[playerid][pChetne] = 0;
	HideName[playerid] = 0;
	...
	if(gPlayerLogged{playerid} == 1)
	{
		...
		switch(reason)
		{
			case 0:
			{
				...
				if(PlayerCuffed[playerid] != 0)
				{
					...
					strcpy(PlayerInfo[playerid][pPrisonedBy], "System", 128);
					PlayerInfo[playerid][pJailTime] += 60*60;
					new szMessage[80+MAX_PLAYER_NAME], stringz[80+MAX_PLAYER_NAME];
					format(szMessage, sizeof(szMessage), "[MAYCHU]: %s da Timeout trong khi bi cong tay.", GetPlayerNameEx(playerid));
					ABroadCast(COLOR_YELLOW, szMessage, 2);
					format(stringz, sizeof(stringz), "[MAYCHU]: %s da vao tu | Ly do: Thoat cong tay (Timeout/Crash).", GetPlayerNameEx(playerid));
					SendClientMessageToAllEx(COLOR_LIGHTRED, stringz);
				}
			}
			case 1:
			{
				...
				if(PlayerCuffed[playerid] != 0)
				{
					...
					PlayerInfo[playerid][pJailTime] += 60*60;
					new szMessage[80+MAX_PLAYER_NAME], stringz[80+MAX_PLAYER_NAME];
					format(szMessage, sizeof(szMessage), "[MAYCHU]: %s da thoat trong khi bi cong tay.", GetPlayerNameEx(playerid));
					ABroadCast(COLOR_YELLOW, szMessage, 2);
					format(stringz, sizeof(stringz), "[MAYCHU]: %s da vao tu | Ly do: Thoat cong tay (LEAVING).", GetPlayerNameEx(playerid));
					SendClientMessageToAllEx(COLOR_LIGHTRED, stringz);
				}
			}
			case 2:
			{
				...
				{
					format(string, sizeof(string), "%s da thoat khoi may chu (Kick/Ban).", GetPlayerNameEx(playerid));
					ProxDetector(30.0, playerid, string, COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW,COLOR_YELLOW);
				}
		...
		if(EventKernel[EventRequest] == playerid)
		{
			...
			ABroadCast( COLOR_YELLOW, "[MAYCHU]: Nguoi choi gui yeu cau da bi disconneted/crashed.", 4 );
		}
		if(EventKernel[EventCreator] == playerid)
		{
			EventKernel[EventCreator] = INVALID_PLAYER_ID;
			ABroadCast( COLOR_YELLOW, "[MAYCHU]: Nguoi choi tao event da bi disconneted/crashed.", 4 );
		}
    ...
    if(!success)
    {
        ...
        foreach(new i : Player)
        {
            ...
            if(!strcmp(ip, pip, true))
            {
				...
				{
					format(string, sizeof(string), "[GORP-Anticheats] %s (IP: %s) da bi khoa tai khoan khi co gang dang nhap vao RCON nhieu lan", GetPlayerNameEx(i), pip);
					Log("logs/ban.log", string);
...
public OnPlayerDeath(playerid, killerid, reason)
{
	...
	{
	    if(!IsACop(playerid) || !IsAHitman(playerid) || !IsAMedic(playerid) || !IsAReporter(playerid) || !IsAGovernment(playerid) || !IsAJudge(playerid) || !IsATaxiDriver(playerid) || !IsATowman(playerid) || !IsARacer(playerid))
	    {
		    if(killerid != INVALID_PLAYER_ID)
		    {
				...
				new szMessage[128];
				format(szMessage, sizeof(szMessage), "Ban da giet {6aa84f}%s{FFFFFF} ({6aa84f}%s{FFFFFF}).", GetPlayerNameEx(playerid), weaponname);
				SendClientMessageEx(killerid, COLOR_WHITE, szMessage);
				
				new szMessagecc[128];
				format(szMessagecc, sizeof(szMessagecc), "Ban da bi giet boi {6aa84f}%s{FFFFFF} ({6aa84f}%s{FFFFFF}).", GetPlayerNameEx(killerid), weaponname);
				SendClientMessageEx(playerid, COLOR_WHITE, szMessagecc);
		    }
	    }
	}
	//new string[1280];
	...
	if(IsPlayerConnected(playerid) && IsPlayerConnected(killerid))
	{
		...
		if(SpoofKill[playerid] >= 4)
		{
			...
			{
				format(string, sizeof(string), "CANH BAO: %s (IP:%s) attempted to spoof kills and has been auto-banned.", GetPlayerNameEx( playerid ), PlayerInfo[playerid][pIP] );
				ABroadCast(COLOR_YELLOW, string, 2);
				PlayerInfo[playerid][pBanned] = 3;
	...
	if(GetPVarInt(playerid, "EventToken") == 0)
	{
	    if(GetPVarInt(playerid, "IsInArena") == -1)
		{
			if(HungerPlayerInfo[playerid][hgInEvent] != 1)
			{
				...
				new Float:mX, Float:mY, Float:mZ;
				GetPlayerPos(playerid, mX, mY, mZ);
				SetPVarInt(playerid, "DaChetEx", 1);
				SetPVarInt(playerid, "ThoiGianChet", 320);
				...
				PlayerInfo[playerid][pDeathPos][0] = mX;
				PlayerInfo[playerid][pDeathPos][1] = mY;
				PlayerInfo[playerid][pDeathPos][2] = mZ;
//				new zone[MAX_ZONE_NAME];
				GetPlayer2DZone(playerid, zone, MAX_ZONE_NAME);
				format(string, sizeof(string), "** Khan Cap Cong Dan (%d) %s Dang Bi Thuong Tai %s (( Su Dung /getpt de chap nhan cap cuu )), Ho yeu cau van chuyen cap cuu ngay lam tuc.",playerid, GetPlayerNameEx(playerid), zone);
				SendGroupMessage(9, TEAM_MED_COLOR, string);
			}
		}
	}
	...
	if(IsPlayerConnected(killerid) && PlayerInfo[killerid][pAdmin] < 2 && GetPlayerState(killerid) == PLAYER_STATE_DRIVER)
	{
		switch(reason)
		{
			case 49: {
			    new szMessage[128];
				format(szMessage, sizeof(szMessage), "[GORP-Anticheats]: %s(ID: %d) da dam xe giet %s (ID: %d) toi chet.", GetPlayerNameEx(killerid), killerid, GetPlayerNameEx(playerid), playerid);
				ABroadCast(COLOR_LIGHTRED, szMessage, 2);
			}
			case 50: if(IsAHelicopter(GetPlayerVehicleID(killerid))) {
			    new szMessage[128];
				format(szMessage, sizeof(szMessage), "[GORP-Anticheats]: %s(ID: %d) da su dung may bay giet chet %s (ID: %d).", GetPlayerNameEx(killerid), killerid, GetPlayerNameEx(playerid), playerid);
				ABroadCast(COLOR_LIGHTRED, szMessage, 2);
			}
			default: switch(GetPlayerWeapon(killerid)) {
				case 32, 28, 29: {
				    new szMessage[128];
					format(szMessage, sizeof(szMessage), "[GORP-Anticheats]: %s(ID: %d) da lai xe giet %s (ID: %d) toi chet.", GetPlayerNameEx(killerid), killerid, GetPlayerNameEx(playerid), playerid);
					ABroadCast(COLOR_LIGHTRED, szMessage, 2);
				}
			}
		}
...
public OnPlayerSpawn(playerid)
{
    ...
    Rapid[playerid] = 0;
    PlayerInfo[playerid][pBuonngu] = 100;
    if(PlayerInfo[playerid][pactive_status] == 1)
	{
		SendClientMessageEx(playerid, COLOR_WHITE,"Tai khoan hien tai chua duoc kich hoat, hay lien he administrator de kich hoat");
		// await task_ms(1500);
...
//	PlayerTextDrawShow(playerid, logo1[playerid]);
	SetPlayerSkillLevel(playerid, WEAPONSKILL_PISTOL, 1);
	SetPlayerSkillLevel(playerid, WEAPONSKILL_MICRO_UZI, 1);
	//PlayerTextDrawShow(playerid, doibung[playerid]);
	//PlayerTextDrawShow(playerid, khat[playerid]);
...
{
	if (GetPVarInt(playerid, "ThiBangLai") > 0)
	{
		new stage = GetPVarInt(playerid, "ThiBangLai");

		switch (stage)
		{
			case 1:
			{
				ShowPlayerDialog(playerid, THONGBAOGPS, DIALOG_STYLE_MSGBOX, "NOTICE", "Hay len xe va khoi dong (N) chay theo vi tri cham do", "Ok", "");
				PlayAudioStreamForPlayer(playerid, "http://web.samp-vn.com/haydidenvitrixuatphat.mp3");
				SetPlayerCheckpoint(playerid, 1677.1582,-1732.3230,13.3828, 2.0);
				SetPVarInt(playerid, "ThiBangLai", 2);
			}
			
			case 2:
			{
				PlayAudioStreamForPlayer(playerid, "http://web.samp-vn.com/haytieptuc.mp3");
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "Ban da hoan thanh vong 1/11");
				SetPlayerCheckpoint(playerid, 1689.5596,-1603.0881,13.3828, 2.0);
				SetPVarInt(playerid, "ThiBangLai", 3);
			}
			case 3:
			{
				PlayAudioStreamForPlayer(playerid, "http://web.samp-vn.com/haytieptuc.mp3");
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "Ban da hoan thanh vong 2/11");
				SetPlayerCheckpoint(playerid, 1849.4264,-1480.5116,13.3694, 2.0);
				SetPVarInt(playerid, "ThiBangLai", 4);
			}
			...
			case 4:
			{
				...
				PlayAudioStreamForPlayer(playerid, "http://web.samp-vn.com/haytieptuc.mp3");
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "Ban da hoan thanh vong 3/11");
				SetPlayerCheckpoint(playerid, 1861.5914,-1340.7770,13.3855, 2.0);
				SetPVarInt(playerid, "ThiBangLai", 5);
			}
			...
			case 5:
			{
				...
				PlayAudioStreamForPlayer(playerid, "http://web.samp-vn.com/haytieptuc.mp3");
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "Ban da hoan thanh vong 4/11");
				SetPlayerCheckpoint(playerid, 2055.7158,-1340.8959,23.8203, 2.0);
				SetPVarInt(playerid, "ThiBangLai", 6);
			}
			case 6:
			{
				
				PlayAudioStreamForPlayer(playerid, "http://web.samp-vn.com/haytieptuc.mp3");
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "Ban da hoan thanh vong 5/11");
				SetPlayerCheckpoint(playerid, 2259.5850,-1300.9060,23.8365, 2.0);
				SetPVarInt(playerid, "ThiBangLai", 7);
			}
			case 7:
			{
				
				PlayAudioStreamForPlayer(playerid, "http://web.samp-vn.com/haytieptuc.mp3");
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "Ban da hoan thanh vong 6/11");
				SetPlayerCheckpoint(playerid, 2279.4473,-1384.2383,24.0162, 2.0);
				SetPVarInt(playerid, "ThiBangLai", 8);
			}
			case 8:
			{
				
				PlayAudioStreamForPlayer(playerid, "http://web.samp-vn.com/haytieptuc.mp3");
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "Ban da hoan thanh vong 7/11");
				SetPlayerCheckpoint(playerid, 2388.9197,-1387.5985,23.8828, 2.0);
				SetPVarInt(playerid, "ThiBangLai", 9);
			}
			case 9:
			{
				
				PlayAudioStreamForPlayer(playerid, "http://web.samp-vn.com/haytieptuc.mp3");
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "Ban da hoan thanh vong 8/11");
				SetPlayerCheckpoint(playerid, 2642.7053,-1457.1410,30.2813, 2.0);
				SetPVarInt(playerid, "ThiBangLai", 10);
			}
			case 10:
			{
				
				PlayAudioStreamForPlayer(playerid, "http://web.samp-vn.com/haytieptuc.mp3");
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "Ban da hoan thanh vong 9/11");
				SetPlayerCheckpoint(playerid, 2643.0640,-1652.2417,10.7031, 2.0);
				SetPVarInt(playerid, "ThiBangLai", 11);
			}
			case 11:
			{
				
				PlayAudioStreamForPlayer(playerid, "http://web.samp-vn.com/haytieptuc.mp3");
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "Ban da hoan thanh vong 10/11");
				SetPlayerCheckpoint(playerid, 2648.3562,-1834.8337,10.8438, 2.0);
				SetPVarInt(playerid, "ThiBangLai", 12);
			}
			case 12:
			{
				
				PlayAudioStreamForPlayer(playerid, "http://web.samp-vn.com/haytieptuc.mp3");
				SendClientMessage(playerid, COLOR_LIGHTBLUE, "Ban da hoan thanh vong 11/11");
				SetPlayerCheckpoint(playerid, 2715.5078,-1847.0586,9.5252, 2.0);
				SetPVarInt(playerid, "ThiBangLai", 13);
			...
			case 13:
			{
				...
				if(health <= 998 && (GetVehicleModel(GetPlayerVehicleID(playerid)) == 458)) 	
				{
				PlayAudioStreamForPlayer(playerid, "http://web.samp-vn.com/thitruot.mp3");
				DeletePVar(playerid, "ThiBangLai");
				DestroyVehicle(playerid);
				PlayerInfo[playerid][pVW] = 0;
				PlayerInfo[playerid][pInt] = 0;
				DisablePlayerCheckpoint(playerid);
				SendClientMessage(playerid, COLOR_GREY, "Ban da thi truot");
				PlayerInfo[playerid][pCash] += 4500;
				SendClientMessage(playerid, COLOR_GREY, "Duoc tra 4500$");
				return 0;
				}
				PlayAudioStreamForPlayer(playerid, "http://web.samp-vn.com/thidau.mp3");
				SetPlayerPos(playerid, 1492.8655,-1739.3660,13.5469);
				PlayerInfo[playerid][pCarLic] = 1;
				PlayerInfo[playerid][pVW] = 0;
				PlayerInfo[playerid][pInt] = 0;
				SendClientMessageEx(playerid, COLOR_YELLOW, "Ban da thi dau bang lai");
				SendClientMessageEx(playerid, COLOR_YELLOW, "/giayphep de kiem tra giay phep lai xe cua ban");
				DestroyVehicle(playerid);
			}
		}
	}	
  ...
  /*  if(CP[playerid] == 4)
    {
        CP[playerid] = 5;
        SendClientMessage(playerid, -1, "Ban da hoan thanh vong vong 1/5 so 8 bang lai");
        SetPlayerCheckpoint(playerid, 1592.4519,-1160.9847,23.9063, 5.0);
        PlayerPlaySound(playerid, 1056, 0.0, 0.0, 0.0);
    }
    if(CP[playerid] == 5)
    {
        CP[playerid] = 6;
        SendClientMessage(playerid, -1, "Ban da hoan thanh vong vong 2/5 so 8 bang lai");
        SetPlayerCheckpoint(playerid, 1623.5383,-1161.2513,23.8962, 5.0);
        PlayerPlaySound(playerid, 1056, 0.0, 0.0, 0.0);
    }
   ...
   if(CP[playerid] == 6)
    {
        CP[playerid] = 7;
        SendClientMessage(playerid, -1, "Ban da hoan thanh vong vong 3/5 so 8 bang lai");
        SetPlayerCheckpoint(playerid, 1644.0858,-1161.0636,23.8979, 5.0);
        PlayerPlaySound(playerid, 1056, 0.0, 0.0, 0.0);
    }
      if(CP[playerid] == 6)
    {
        CP[playerid] = 7;
        SendClientMessage(playerid, -1, "Ban da hoan thanh vong vong 4/5 so 8 bang lai");
        SetPlayerCheckpoint(playerid, 1682.1631,-1160.7152,23.6563, 5.0);
        PlayerPlaySound(playerid, 1056, 0.0, 0.0, 0.0);
    }
      if(CP[playerid] == 7)
    {
        CP[playerid] = 0;
        SendClientMessage(playerid, -1, "/thixong de ve nhan ho so thi dau bang lai xe");
        SendClientMessage(playerid, -1, "Ban da hoan thanh vong vong 5/5 so 8 bang lai");
        PlayerPlaySound(playerid, 1056, 0.0, 0.0, 0.0);
        DisablePlayerCheckpoint(playerid);
    }*/
	...
	for(new h = 0; h < MAX_POINTS; h++)
	{
		if(Points[h][Type] == 3 && GetPVarInt(playerid, "CrateDeliver") == 1 && IsPlayerInRangeOfPoint(playerid, 6.0, 2166.3772,-1675.3829,15.0859))
		{
	    	...
	    	{
			   	format(string, sizeof(string), "[GORP-Anticheats]: %s (ID %d) Dang su dung Teleport.", GetPlayerNameEx(playerid), playerid);
			   	ABroadCast( COLOR_YELLOW, string, 2 );
	...
	if(GetPVarInt(playerid, "TruckDeliver") > 0 && gPlayerCheckpointStatus[playerid] != CHECKPOINT_RETURNTRUCK)
	{
		...
		{
  			new string[128];
			format(string, sizeof(string), "[MAYCHU]: %s (ID %d) is possibly teleport truck/boat running.", GetPlayerNameEx(playerid), playerid);
  			ABroadCast( COLOR_YELLOW, string, 2 );
		...
		{
			SetPlayerCheckpoint(playerid, 2508.9888,-2120.3621,13.5469, 5);
			GameTextForPlayer(playerid, "~w~Dia diem ~r~OCEAN DOCKS", 5000, 1);
			SendClientMessageEx(playerid, COLOR_WHITE, "HUONG DAN: Quay tro lai OCEAN DOCKS (diem checkpoint tren radar).");
		}
		else
		{
			SetPlayerCheckpoint(playerid, 2098.6543,-104.3568,-0.4820, 5);
			GameTextForPlayer(playerid, "~w~Dia diem ~r~Ben cang Palamino", 5000, 1);
			SendClientMessageEx(playerid, COLOR_WHITE, "HUONG DAN: Quay tro lai ben cang Palamino (diem checkpoint tren radar).");
		}
	...
	if(gPlayerCheckpointStatus[playerid] == CHECKPOINT_DELIVERY)
	{
		...
		{
  			new string[128];
			format(string, sizeof(string), "[GORP-Anticheats] %s (ID %d) Dang su dung Teleport/Trucker.", GetPlayerNameEx(playerid), playerid);
  			ABroadCast( COLOR_YELLOW, string, 2 );
    		// format(string, sizeof(string), "%s (ID %d) is possibly teleport truckrunning.", GetPlayerNameEx(playerid), playerid);
	    	// Log("logs/hack.log", string);
		}
		...
		SetPlayerCheckpoint(playerid, 2508.9888,-2120.3621,13.5469, 5);
		GameTextForPlayer(playerid, "~w~Dia diem ~r~OCEAN DOCKS", 5000, 1);
		SendClientMessageEx(playerid, COLOR_WHITE, "HUONG DAN Quay lai OCEAN DOCKS (diem checkpoint tren radar).");
	...
	if(GetPVarInt(playerid, "Pizza") > 0 && IsPlayerInRangeOfPoint(playerid, 5.0, HouseInfo[GetPVarInt(playerid, "Pizza")][hExteriorX], HouseInfo[GetPVarInt(playerid, "Pizza")][hExteriorY], HouseInfo[GetPVarInt(playerid, "Pizza")][hExteriorZ]) && GetPlayerInterior(playerid) == HouseInfo[GetPVarInt(playerid, "Pizza")][hExtIW] && GetPlayerVirtualWorld(playerid) == HouseInfo[GetPVarInt(playerid, "Pizza")][hExtVW])
	{
		...
		{
			format(string, sizeof(string), "[GORP-Anticheats] %s (ID %d) Dang su dung Teleport/Pizza", GetPlayerNameEx(playerid), playerid);
  			ABroadCast( COLOR_YELLOW, string, 2 );
		...
		new rand=random(50);
		new sotien = 30;
		//PlayAudioStreamForPlayer(playerid, "http://web.samp-vn.com/bandagiaobanh.mp3");
	...
	else
	{
		switch (gPlayerCheckpointStatus[playerid])
		{
			...
			{
			    if(IsPlayerInRangeOfPoint(playerid, 6, 2508.7222,-2118.6780,13.5469) || IsPlayerInRangeOfPoint(playerid, 6, 2098.6543,-104.3568,-0.4820))
			    {
				    new vehicleid = GetPlayerVehicleID(playerid);
			...
			{
			    if(!IsPlayerInRangeOfPoint(playerid, 6, 2508.9888,-2120.3621,13.5469) && !IsPlayerInRangeOfPoint(playerid, 6, 2098.6543,-104.3568,-0.4820))
				...
				if(level >= 0 && level <= 100) payment = 750;
				else if(level >= 101 && level <= 200) payment = 850;
				else if(level >= 201 && level <= 300) payment = 950;
				else if(level >= 301 && level <= 400) payment = 1000;
				else if(level >= 1000) payment = 1100;
				...
				{
				    distancepay = floatmul(GetDistance(2508.7222,-2118.6780,13.5469, TruckerDropoffs[route][PosX], TruckerDropoffs[route][PosY], TruckerDropoffs[route][PosZ]), 1.5);
				}
				*/
				...
				if(TruckDeliveringTo[vehicleid] != INVALID_BUSINESS_ID) {
					...
					if(truckdeliver == 5) // Weapons
					{
						...
						{
							if(level >= 0 && level <= 100) // truck level 1
							{
								SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Ban nhan duoc mien phi Bat cho viec van chuyen vu khi trai phep.");
								GivePlayerValidWeapon(playerid, 5, 60000);
							}
							else if(level >= 101 && level <= 200) // truck level 2
							{
									
								SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Ban nhan duoc mien phi Shovel cho viec van chuyen vu khi trai phep.");
								GivePlayerValidWeapon(playerid, 6, 60000);
							}
							else if(level >= 201 && level <= 300) // truck level 3
							{
								SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Ban nhan duoc mien phi 9mm cho viec van chuyen vu khi trai phep.");
								SendClientMessageEx(playerid, -1, "Hay su dung vu khi nhu mot nguoi co y thuc nhe");
								GivePlayerValidWeapon(playerid, 22, 2);
							}
                            ...
                            else if(level >= 401 && level <= 1000) // truck level 5
                            {
					...
					if(truckdeliver == 6) // Drugs
					{
						...
						{
		                    SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Ban da nhan duoc 4 pot & 4 cho viec van chuyen ma tuy bat hop phap.");
					...
					if(truckdeliver == 7) // Illegal materials
					{
						if(level >= 0 && level <= 100)
						{
							SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Ban da nhan duoc 25 cho viec van chuyen vat lieu bat hop phap.");
							PlayerInfo[playerid][pMats] += 25;
						}
						else if(level >= 101 && level <= 200)
						{
		                    SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Ban da nhan duoc 50 cho viec van chuyen vat lieu bat hop phap.");
						...
						{
		                    SendClientMessageEx(playerid, COLOR_LIGHTBLUE, "* Ban da nhan duoc 150 cho viec van chuyen vat lieu bat hop phap.");
			...
			case CHECKPOINT_HITMAN:
			{
			    ...
			    DisablePlayerCheckpoint(playerid);
			    gPlayerCheckpointStatus[playerid] = CHECKPOINT_NONE;
			    SendClientMessageEx(playerid, COLOR_GRAD2, "  Su dung (Y) de vao HQ.");
			}
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	if(newkeys & KEY_YES)
	{
		if(IsPlayerInRangeOfPoint(playerid,5.0,2211.2983,-2664.5122,13.5469))
		{
			ShowPlayerDialog(playerid, DIALOG_XINVIECTRUCK, DIALOG_STYLE_LIST, "Cong viec", "Xin Viec[Lv2+]", "Chon","{FFFFFF}Huy");
		}
	}
	if(newkeys & KEY_YES)
	{
        if(IsPlayerInRangeOfActor(playerid, npccityhall))
	    {
    	    ShowPlayerDialog(playerid, DIALOG_CITYHALL, DIALOG_STYLE_LIST, "> CITY HALL","Mua bai dau xe(500$)\nDong ve phat", "Chon", "Thoat");
		    return 1;
        }
	}
...
/*	if(newkeys & KEY_YES)
	{
	    if(IsPlayerInRangeOfPoint(playerid, 5.0,1919.5537,-1773.6747,13.5828))
	    {
			return cmd_mua(playerid, "");
	    }
	}*/
	...
	if(newkeys & KEY_YES)
	{
		if(IsAMedic(playerid)) {
			if(IsPlayerInRangeOfPoint(playerid, 3.0, -2031.3843,-114.7203,1035.1719))
			{
				ShowPlayerDialog(playerid, DIALOG_MUAMEDKIT,DIALOG_STYLE_TABLIST_HEADERS,"Trao Doi Med Kit","Medkit\nMua Medkit Medic","Chon","Huy");
	    	}
		}
	}
	if(newkeys & KEY_NO)
	{
	    if(IsPlayerInRangeOfPoint(playerid, 3.0,2789.0022,-1415.1759,16.2500))
	    {
			ShowPlayerDialog(playerid, DIALOG_BUYWEAPONS,DIALOG_STYLE_TABLIST_HEADERS,"CUA HANG VU KHI","Loai\tHinh thuc\nVu Khi\t[Tien]","Chon","Huy");
	    }
	}
	if(newkeys & KEY_YES)
	{
	...
	if(newkeys & KEY_YES)
	{
	    for(new i = 0; i < sizeof(DDoorsInfo); i++) {
			...
			else if (IsPlayerInRangeOfPoint(playerid,3.0,DDoorsInfo[i][ddInteriorX], DDoorsInfo[i][ddInteriorY], DDoorsInfo[i][ddInteriorZ]) && PlayerInfo[playerid][pVW] == DDoorsInfo[i][ddInteriorVW])
	    ...
	    for(new i = 0; i < sizeof(FamilyInfo); i++) {
	        ...
	        return cmd_enter(playerid, "");
	        else if(IsPlayerInRangeOfPoint(playerid,3.0,FamilyInfo[i][FamilyExit][0], FamilyInfo[i][FamilyExit][1], FamilyInfo[i][FamilyExit][2]) && PlayerInfo[playerid][pVW] == FamilyInfo[i][FamilyVirtualWorld])
            return cmd_exit(playerid, "");
		}
  		for(new i = 0; i < sizeof(HouseInfo); i++) {
            ...
            return cmd_enter(playerid, "");
            else if (IsPlayerInRangeOfPoint(playerid,3.0,HouseInfo[i][hInteriorX], HouseInfo[i][hInteriorY], HouseInfo[i][hInteriorZ]) && PlayerInfo[playerid][pVW] == HouseInfo[i][hIntVW])
            return cmd_exit(playerid, "");
    	}
	    for(new i = 0; i < sizeof(Businesses); i++) {
	        if (IsPlayerInRangeOfPoint(playerid,3,Businesses[i][bExtPos][0], Businesses[i][bExtPos][1], Businesses[i][bExtPos][2]))
            return cmd_enter(playerid, "");
            else if (IsPlayerInRangeOfPoint(playerid,3,Businesses[i][bIntPos][0], Businesses[i][bIntPos][1], Businesses[i][bIntPos][2]))
            return cmd_exit(playerid, "");
		}
	}
	if(newkeys & KEY_YES)
	{
		if(IsPlayerInRangeOfPoint(playerid, 3.0, 2786.4319,-1417.5588,16.2500))
		{
			ShowPlayerDialog(playerid,2730,DIALOG_STYLE_LIST,"Cho den","Trao Doi La Can Sa","{ae0202}Chon","{FFFFFF}Huy");
		}
	}
	if(newkeys & KEY_YES)
	{
	        cmd_haicanzz(playerid);
	}
	if(newkeys & KEY_WALK)
	{
	    if(IsPlayerInRangeOfPoint(playerid, 5.0, 1267.9139,-1266.3197,13.3957))
	    {
	...
	if (newkeys & KEY_YES)
    {
        ...
        /*new Float:PosXActorNguoivanchuyen, Float:PosYActorNguoivanchuyen, Float:PosZActorNguoivanchuyen;
		GetActorPos(ActorNguoivanchuyen, PosXActorNguoivanchuyen, PosYActorNguoivanchuyen, PosZActorNguoivanchuyen);
        if(IsPlayerInRangeOfPoint(playerid, 5.0, PosXActorNguoivanchuyen, PosYActorNguoivanchuyen, PosZActorNguoivanchuyen))
        {
       		ShowJoinJob(playerid);
			return 0;
        }
        new Float:PosXActorThothucong, Float:PosYActorThothucong, Float:PosZActorThothucong;
		GetActorPos(ActorThothucong, PosXActorThothucong, PosYActorThothucong, PosZActorThothucong);
        if(IsPlayerInRangeOfPoint(playerid, 5.0, PosXActorThothucong, PosYActorThothucong, PosZActorThothucong))
        {
			ShowJoinJob(playerid);
			return 0;
        }
        new Float:PosXActorThan, Float:PosYActorThan, Float:PosZActorThan;
		GetActorPos(ActorThan, PosXActorThan, PosYActorThan, PosZActorThan);
        if(IsPlayerInRangeOfPoint(playerid, 5.0, PosXActorThan, PosYActorThan, PosZActorThan))
        {
			ShowJoinJob(playerid);
			return 0;
        }*/
        ...
        if(IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
       	{
	    	...
	    	for(new i = 0;i < MAX_PLAYERS;i++){
			    for(new d = 0 ; d < MAX_PLAYERVEHICLES; d++){
					if(vehicleid == PlayerVehicleInfo[i][d][pvId] && i != playerid) cothe = 0;
				}
	    	}
	    	...
	    	if(PlayerInfo[playerid][pVehicleKeysFrom] != INVALID_PLAYER_ID){
				...
				{
		            new d = PlayerInfo[playerid][pVehicleKeys];
		            if(PlayerVehicleInfo[ownerid][d][pvId] != INVALID_PLAYER_VEHICLE_ID)
		            	if(PlayerVehicleInfo[ownerid][d][pvId] == vehicleid) cothe = 1;
				}
	    	}
	    	new engine,lights,alarm,doors,bonnet,boot,objective;
	    	GetVehicleParamsEx(vehicleid,engine,lights,alarm,doors,bonnet,boot,objective);
		    if(cothe == 0)
		    {
	            ...
	            else ChatHoTro(playerid,255,"Day khong phai xe cua ban ~r~/bekhoa~w~ de be khoa xe",5000);
		    }
		    if(cothe == 1)
		    {
	            //new engine,lights,alarm,doors,bonnet,boot,objective;
	            ...
	            new string[128];
				PlayerTextDrawHide(playerid, EnterVeh[playerid]);
	            GameTextForPlayer(playerid, "~y~KHOI DONG CO XE", 3000, 3);
	...
	else if(GetPlayerSpecialAction(playerid) == SPECIAL_ACTION_DRINK_SPRUNK && (newkeys & KEY_FIRE))
	{
		if(GetPVarInt(playerid, "DrinkCooledDown") == 1 && GetPVarInt(playerid, "UsingSprunk") == 1)
		{
			...
			if(cHealth < 100)
			{
				...
				PlayerInfo[playerid][pKhatNuoc] += 10;
				PlayerInfo[playerid][pKhatNuocTimer] = 0;
            	PlayerInfo[playerid][pThieuNuocTimer] = 0;
			}
			else
			{
				DeletePVar(playerid, "UsingSprunk");
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
	...
	if(IsPlayerInAnyVehicle(playerid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
	{
		...
		{
			if(GetVehicleModel(vehicleid) != 481 || GetVehicleModel(vehicleid) != 509 || GetVehicleModel(vehicleid) != 510 || DynVeh[vehicleid] != -1 && DynVehicleInfo[DynVeh[vehicleid]][gv_iType] != 1 && GetVehicleModel(vehicleid) != 592)
			{
				PlayerTextDrawShow(playerid, EnterVeh[playerid]);
			}
		}
		if(PlayerInfo[playerid][pCarLic] == 0)
		{
			...
			if(GetVehicleModel(vehicleid) == 481 || GetVehicleModel(vehicleid) == 509 || GetVehicleModel(vehicleid) == 510 || DynVeh[vehicleid] != -1 && DynVehicleInfo[DynVeh[vehicleid]][gv_iType] == 1 && GetVehicleModel(vehicleid) == 592) return SendClientMessageEx(playerid,COLOR_WHITE,"Phuong tien nay khong can giay phep lai xe");
			SendClientMessage(playerid, COLOR_GREY, "Ban chua co giay phep lai xe, Ban co the bi bat");
			/*format(str, sizeof(str),"{3e28d2}[CAMERA] {FFFFFF}Phuong tien %s Khong co giay phep lai xe dang dieu khien xe trai phep tren duong %s",GetVehicleName(veh),zone);
			foreach(new i: Player)
			{
				if(PlayerInfo[i][pMember] == iGroupID && iRank >= arrGroupData[iGroupID][g_iRadioAccess]) 
				{	
					SendClientMessageEx(i, arrGroupData[iGroupID][g_hRadioColour] * 256 + 255, str);
				}
			}*/
		}
	}
    if(newstate == PLAYER_STATE_DRIVER || newstate == PLAYER_STATE_PASSENGER)
	{
 		if(!IsPlayerEntering{playerid})
		{
			if(GetPVarInt(playerid, "TeleportWarnings") == 3) {
				new string[128];
		    	format(string, sizeof(string), "[MAYCHU]: %s (ID: %d) may have just teleported into a vehicle.", GetPlayerNameEx(playerid), playerid);
				ABroadCast(COLOR_YELLOW, string, 2);
	...
	if(newstate == PLAYER_STATE_DRIVER)
	{
		...
		if(GetCarBusiness(newcar) != INVALID_BUSINESS_ID && PlayerInfo[playerid][pAdmin] < 1337)
        {
			...
			//ShowPlayerDialog(playerid,LENXESHOP,DIALOG_STYLE_LIST,"Cua hang xe","Test xe\nMua xe","Chon","Huy bo");
		...
		if((engine == VEHICLE_PARAMS_UNSET || engine == VEHICLE_PARAMS_OFF) && GetVehicleModel(newcar) != 509 && GetVehicleModel(newcar) != 481 && GetVehicleModel(newcar) != 510 && (DynVeh[newcar] != -1 && GetVehicleModel(newcar) == 592 && DynVehicleInfo[DynVeh[newcar]][gv_iType] != 1) ) {
			SendClientMessageEx(playerid, COLOR_WHITE, "This vehicle's engine is not running - if you wish to start it, type /xe khoidong.");
		}
...
{
	if(TestingModel[playerid] == 1)
	{
		SendClientMessageEx(playerid, COLOR_YELLOW, "Vi ban da xuong xe nen ban thoi gian thu xe da ket thuc.");
		new car = GetPlayerVehicleID(playerid);
		DestroyVehicle(car);
		SetPlayerPos(playerid, testcarpos1[playerid],testcarpos2[playerid],testcarpos3[playerid]);
...
public OnPlayerCommandPerformed(playerid, cmdtext[], success)
{
    ...
    if(!success)
    {
    	//PlayAudioStreamForPlayer(playerid, "http://web.samp-vn.com/trogiupxemlenh.mp3");
        format(string,sizeof(string),"{ff7700}[!]GO:RP{FFFFFF}:{FFFFFF}Lenh {ff0000}%s{ffffff} Khong Ton Tai, ban vui long [/trogiup] de xem huong dan", cmdtext);
        SendClientMessageEx(playerid, COLOR_GREY, string);
    }
    return 1;
}
...
public OnPlayerCommandReceived(playerid, cmdtext[]) {
	...
	{
		new string[128];
		format(string,sizeof(string),"[MAYCHU]: %s (ID: %d) co the dang quang cao may chu: '{AA3333}%s{FFFF00}'.", GetPlayerNameEx(playerid), playerid, cmdtext);
		ABroadCast(COLOR_YELLOW, string, 2);
		Log("logs/hack.log", string);
		return 0;
	}
...
public OnPlayerText(playerid, text[])
{
	...
	if(GetPVarInt(playerid,"Injured") != 0)
	{
		SendClientMessageEx(playerid, COLOR_RED, "Ban khong the noi chuyen khi dang bi thuong su dung (/l) de noi chuyen");
		return 0;
	}
	new string[128];
	if(GetPVarInt(playerid,"Injured") != 0 && PlayerInfo[playerid][pChetne] != 0)
	{
		...
		if(GetPVarString(playerid, "LastText", laststring, 128)) {
			if(!strcmp(laststring, text, true)) {
				...
				if(TextSpamTimes[playerid] == 2) {
					...
					TextSpamTimes[playerid] = 0;
					format(string, sizeof(string), "[MAYCHU]: %s (ID %d) is spamming with: %s", GetPlayerNameEx(playerid), playerid, text);
					ABroadCast(COLOR_YELLOW, string, 2);
				}
			}
		}
	...
	{
		format(string,sizeof(string),"[MAYCHU]: %s (ID: %d) co the dang quang cao may chu: '{AA3333}%s{FFFF00}'.", GetPlayerNameEx(playerid), playerid, text);
		ABroadCast(COLOR_YELLOW, string, 2);
		Log("logs/hack.log", string);
		return 0;
	}
	...
	if(Mobile[playerid] != INVALID_PLAYER_ID)
	{
	    ...
	    SetPlayerChatBubble(playerid,string,COLOR_PURPLE,60.0,2000);
