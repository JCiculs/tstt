...
public OnPlayerLoad(playerid)
{
	...
	if(PlayerInfo[playerid][pReg] == 0)
	{
		for(new v = 0; v < MAX_PLAYERVEHICLES; v++)
		{
			...
			PlayerVehicleInfo[playerid][v][pvHealth] = 0.0;
			PlayerVehicleInfo[playerid][v][pvBug] = 0;
		...
		PlayerInfo[playerid][pHasBean] = 0;
		...
		for(new i = 0; i < 3; i++) PlayerInfo[playerid][pDeathPos] = 0.0;
	}
	...
	if(PlayerInfo[playerid][pVIPExpire] > 0 && (1 <= PlayerInfo[playerid][pDonateRank] <= 3) && (PlayerInfo[playerid][pVIPExpire] < gettime()) && PlayerInfo[playerid][pAdmin] < 2)
	{
	    ...
	    format(string, sizeof(string), "[MAYCHU] Please check person %s as their VIP may have expired.", GetPlayerNameEx(playerid));
	...
	if(PlayerInfo[playerid][pAdmin] != 0 && PlayerInfo[playerid][pAdmin] != 1 && PlayerInfo[playerid][pAdmin] != 2 && PlayerInfo[playerid][pAdmin] != 3 && PlayerInfo[playerid][pAdmin] != 4 &&PlayerInfo[playerid][pAdmin] != 5 && PlayerInfo[playerid][pAdmin] != 6 && PlayerInfo[playerid][pAdmin] != 7 && PlayerInfo[playerid][pAdmin] != 8)
	{
		new name[MAX_PLAYER_NAME];
		GetPlayerName(playerid, name, sizeof(name));
		format(string, sizeof(string), "[MAYCHU] %s dang co gang dang nhap Admin Level %d.", GetPlayerNameEx(playerid), PlayerInfo[playerid][pAdmin]);
		ABroadCast( COLOR_YELLOW, string, 4 );
	...
	if (PlayerInfo[playerid][pAdmin] > 0)
	{
		...
		else
		{
			...
			format(string, sizeof(string), "* Tai khoan dang nhap la %s{FFFFFF}.", GetStaffRank(playerid));
			SendClientMessageEx(playerid, COLOR_WHITE, string);
			format(string, sizeof(string), "* %s da dang nhap %s{FFFFFF}.", GetPlayerNameEx(playerid), GetStaffRank(playerid));
		}
	...
	PlayAudioStreamForPlayer(playerid, "http://web.samp-vn.com/chaomungbanquaytrolai.mp3");
	LoadingPlayer[playerid] = SetTimerEx("OnPlayerLoading", TIMELOADING*1000, 0, "ii",playerid);
	LoadingPlayer[playerid] = 1;
	TogglePlayerControllable(playerid, 0);
	TextDrawHideForPlayer(playerid, logocustom[0]);
	SendClientMessage(playerid, -1, "Dang kiem tra tai khoan");
	...
	TextDrawHideForPlayer(playerid, Notifix[3]);
	...
	{
		SendClientMessageEx(playerid, COLOR_WHITE, "FAMILY:");
		for(new i = 1; i <= 3; i++)
		{
	...
	{
		format(string, sizeof(string), "{ffe135}FACTION: %s{FFFFFF}", arrGroupData[PlayerInfo[playerid][pMember]][g_szGroupMOTD]);
		SendClientMessageEx(playerid, COLOR_WHITE, string);
	}
	CountFlags(playerid);
	...
	TogglePlayerControllable(playerid, 0);
	SetTimerEx("PlayerLoadGame",5000,false,"i",playerid);
	new version[64];
	GetPlayerVersion(playerid,version,64);
	GetPlayerIp(playerid, ip, sizeof(ip));
	if(GetPVarInt(playerid, "NotAndroid") == 0)
	{
		ShowHunger_MB(playerid);
	 	format(string, sizeof(string), "SERVER: %s (%d) [IP: %s] - ver Mobile", GetPlayerNameExt(playerid), playerid, ip);
	}
	else 
	{
		ShowHunger_PC(playerid);
		format(string, sizeof(string), "SERVER: %s (%d) [IP: %s] - ver PC [%s]", GetPlayerNameExt(playerid), playerid, ip, version);
	}
	foreach(new i: Player)
	{
...
/*	new nxtlevel = PlayerInfo[playerid][pLevel]+1;
	new expamount = nxtlevel;
    if(PlayerInfo[playerid][pExp] >= expamount)
	{
		format(string, sizeof(string), "~g~LEN CAP~n~~w~Ban da thang cap len %d", nxtlevel);
		PlayerPlaySound(playerid, 1052, 0.0, 0.0, 0.0);
		PlayerInfo[playerid][pLevel]++;
		PlayerInfo[playerid][pExp] = PlayerInfo[playerid][pExp]-expamount;
		PlayerInfo[playerid][gPupgrade] = PlayerInfo[playerid][gPupgrade]+2;
		GameTextForPlayer(playerid, string, 5000, 1);
		format(string, sizeof(string), "Ban da len level %d va da dat duoc %i diem nang cap! (/nangcap) de su dung chung.", nxtlevel, PlayerInfo[playerid][gPupgrade]);
		SendClientMessageEx(playerid, COLOR_GRAD1, string);
		SetPlayerScore(playerid, PlayerInfo[playerid][pLevel]);
	}*/
	/*else if(PlayerInfo[playerid][pLevel] == 1){
		if(PlayerInfo[playerid][pExp] >= 4){
			format(string, sizeof(string), "~g~LEN CAP~n~~w~Ban da thang cap len %d", nxtlevel);
			PlayerPlaySound(playerid, 1052, 0.0, 0.0, 0.0);
			PlayerInfo[playerid][pLevel]++;
			PlayerInfo[playerid][pExp] = PlayerInfo[playerid][pExp]-expamount;
	...
	PlayerInfo[playerid][pWeedObject] = 0;
	if(PlayerInfo[playerid][pDeathPos][0] != 0.0 && PlayerInfo[playerid][pDeathPos][1] != 0.0 && PlayerInfo[playerid][pDeathPos][2] != 0.0)
	{
...
forward PlayerLoadGame(playerid);
public PlayerLoadGame(playerid)
{
	TogglePlayerControllable(playerid, 1);
	return 1;
