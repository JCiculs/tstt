		...
		if(GetPVarInt(playerid, "ClickMapa") > gettime())
		{
		    if(PlayerInfo[playerid][pAdmin] == 0) {
				if(!IsPlayerInRangeOfPoint(playerid, 30.0, GetPVarFloat(playerid, "PX"), GetPVarFloat(playerid, "PY"), GetPVarFloat(playerid, "PZ")))
				{
					...
					new string[1280];
					format(string,sizeof(string),"[GORP-Anticheats] %s (ID: %d) co the dang su dung teleport.",GetPlayerNameEx(playerid), playerid);
...
task CheckHackFly[500]()
{
	...
	{
		new string[128];
		pSpeed[playerid] = GetSpeedPlayer(playerid);
		if(pSpeed[playerid] >= 199.0 && PlayerInfo[playerid][pAdmin] < 2)
		{
			if(ReloadTime[playerid] < gettime())
			{
			    format(string, sizeof(string), "[GORP-Anticheats] %s co the dang hack Slap .", GetPlayerNameEx(playerid), GetSpeedPlayer(playerid));
			    ABroadCast(COLOR_LIGHTRED, string, 2);
...
task AntiSlap[500]()
{
    foreach(new playerid : Player) {
		if(PlayerInfo[playerid][pAdmin] < 2 && GetPVarInt(playerid, "spam1") == 0)
	    {
			...
			{
				format(string,sizeof(string),"[GORP-Anticheats] %s (ID: %d) co the dang Surfing Foot.",GetPlayerNameEx(playerid), playerid);
				ABroadCast(COLOR_LIGHTRED , string, 2);        
		...
		if(GetPlayerVehicleID(playerid) != 0) {
		    if(TrollCar[playerid] != GetPlayerVehicleID(playerid))
			{
			    if(GetPVarInt(playerid, "KhongHacking") == 0 && GetPVarInt(playerid, "spam3") == 0)
			    {
	                ...
	                //==================//
	                new string[256];
					format(string, sizeof(string), "[GORP-Anticheats]: %s (ID %d) co the dang su dung Hack Troll Car.", GetPlayerNameEx(playerid), playerid);
					ABroadCast(COLOR_LIGHTRED , string, 2);
					//==================//
				}
			}
...
/*task Sleeping[300000]() 
{
    foreach(new i: Player) 
    {
        new rand = random(5,10);
        PlayerInfo[i][pBuonngu] -= rand;
        if(PlayerInfo[i][pBuonngu] <= 0)
        {
            PlayerInfo[i][pBuonngu] = 0;
            SendClientMessageEx(i, COLOR_GREY, "Ban dang rat met vi buon ngu, hay di ngu ngay de phuc hoi.");
        }
    }
}*/


/* task doikhatt[300000]() {
	foreach(new i: Player) {
//		new string[10];
		new rand = random(5,10);
		PlayerInfo[i][pHunger] -= rand;
		PlayerInfo[i][pKhatNuoc] -= rand;
		if(PlayerInfo[i][pHunger] <= 0){
			...
			SendClientMessageEx(i, COLOR_GREY, "Ban dang rat doi bung");
		}
		if(PlayerInfo[i][pKhatNuoc] <= 0) {
			PlayerInfo[i][pKhatNuoc] = 0;
	        ApplyAnimation(i, "CRACK", "crckdeth2", 4.1, 1, 1, 1, 1, 1, 1);
	        new Float:HP;
	        GetPlayerHealth(i, HP);
	        SetPlayerHealth(i, HP - 10.0);
			SendClientMessageEx(i, COLOR_GREY, "Ban dang rat khat nuoc");
		}
	}
} */

task weathertime[30000]() {
...
new
	Float:testcarpos1[MAX_PLAYERS],
	Float:testcarpos2[MAX_PLAYERS],
	Float:testcarpos3[MAX_PLAYERS],
	TestingModel[MAX_PLAYERS];

timer CancelTestDrive[180000](playerid) {
	if(TestingModel[playerid] == 0) return 1;
	SendClientMessageEx(playerid, COLOR_YELLOW, "Thoi gian thu xe da ket thuc.");
...
task ProductionUpdate[600000]()
{
	...
	foreach(new i: Player)
	{
		...
		{
			SendClientMessageEx(i, COLOR_LIGHTBLUE, "Ban da online duoc 1 tieng, hay den cac tru atm de lanh luong /lanhluong");
		}
		case 1:
		if(PlayerInfo[i][pLevel] < 3)
		{
			SendClientMessageEx(i, COLOR_LIGHTRED, "Neu ban cam thay may chu co nhieu dieu thu vi xin hay dung quen gioi thieu ban be nhe");

		}
		case 2:
		if(PlayerInfo[i][pLevel] < 3)
		{
			SendClientMessageEx(i, COLOR_LIGHTRED, "Ban muon quang cao mon do cua ban /ads");

		}
		case 3:
		if(PlayerInfo[i][pLevel] < 3)
		{
			SendClientMessageEx(i, COLOR_LIGHTRED, "Hay tham gia discord thuong xuyen de cap nhap thong tin");

		}
		case 4:
		if(PlayerInfo[i][pLevel] > 1)
		{
			SendClientMessage(i, COLOR_YELLOW, "Jobs [Tieu Phu] Co ty le roi ra go nho, go nho co the che tao vu khi, xay dung nha cua");
		}
		case 5:
		if(PlayerInfo[i][pLevel] > 1)
		{
			SendClientMessage(i, COLOR_YELLOW, "Help: Neu ban thac nhieu dieu chua tim hieu het hay /trogiup");
		}
	}
		    ...
		    if(GetPlayerPing(i) > MAX_PING)
		    {
		        if(playerTabbed[i] == 0)
		        {
					if(GetPVarInt(i, "BeingKicked") != 1)
					{
						...
						{
							format(string, sizeof(string), "[MAYCHU] %s da tu dong bi kick voi %d ping (maximum: "#MAX_PING").", GetPlayerNameEx(i), ping);
	...
	foreach(new i: Player)
	{
	    ...
	    if(GetPVarType(i, "Injured"))
	    {
	        ...
	        if(GetPVarInt(i, "EMSAttempt") != 0)
			{
				...
				{
					if(GetPlayerAnimationIndex(i) != 746) ClearAnimations(i), ApplyAnimation(i, "SWEET", "Sweet_injuredloop", 4.0, 0, 1, 1, 1, 0, 1);
					...
					new string[200];
                    format(string, sizeof(string), "((DANG BI THUONG))");
					PlayerTextDrawShow(i, NotiEms[i]);
					ApplyAnimation(i, "SWEET", "Sweet_injuredloop", 4.1, 0, 0, 0, 300000, 0);
                    SetPlayerChatBubble	(i,string,COLOR_LIGHTRED,50.0,10000);
				}
				if(GetPVarInt(i, "EMSAttempt") == 1)
				{
					if(GetPlayerAnimationIndex(i) != 746) ClearAnimations(i), ApplyAnimation(i, "SWEET", "Sweet_injuredloop", 4.1, 0, 0, 0, 300000, 0);
	    			if(!GetPVarType(i, "StreamPrep") && !IsPlayerInRangeOfPoint(i, 3.0, GetPVarFloat(i,"MedicX"), GetPVarFloat(i,"MedicY"), GetPVarFloat(i,"MedicZ")) && !GetPVarInt(i, "OnStretcher"))
	    			{
				...
				if(GetPVarInt(i, "EMSAttempt") == 4)
				{
	    			if(IsPlayerInAnyVehicle(i))
	    			{
	        			new ambmodel = GetPlayerVehicleID(i);
	        			if(IsAHelicopter(ambmodel))
	        			{
	    					//GameTextForPlayer(i, "~g~Dich vu~n~~w~Dang cho EMS dua den benh vien...", 5000, 3);
						}
						else
				...
				if(health <= 5)
				{
					PlayerTextDrawHide(i, NotiEms[i]);
...
task ServerHeartbeat[1000]() {
       ...
       foreach(new i: Player)
     {
         if(GetPlayerSpecialAction(i) == SPECIAL_ACTION_USEJETPACK && JetPack[i] == 0 && PlayerInfo[i][pAdmin] < 1)
         {
            ...
            format( string, sizeof( string ), "[GORP-Anticheats] %s (ID %d) co the dang hack jetpack.", GetPlayerNameEx(i), i);
            ABroadCast( COLOR_LIGHTRED, string, 2 );
            format(string, sizeof(string), "[GORP-Anticheats] %s (ID %d) co the dang hack jetpack.", GetPlayerNameEx(i), i);
            Log("logs/hack.log", string);
            SetPlayerHealth(i, 0.0);
            SendClientMessage(i, -1, "Ban da chet vi nghi van hack JETPACK");
         }
     }
	if(++AdminWarning == 15) {
	...
	foreach(new i: Player)
	{
		...
		if (PlayerInfo[i][pAdmin] < 2 || HelpingNewbie[i] != INVALID_PLAYER_ID)
		{
			...
			{
				if (++PlayerInfo[i][pHungerTimer] >= 900 && PlayerInfo[i][pHunger] > 0) // 72 giay la bi -2 trang thai, tuc la 100 hunger thi 1 tieng moi doi bung
				{
					...
					if (PlayerInfo[i][pHunger] == 0)
					{
						PlayAudioStreamForPlayer(i, "http://web.samp-vn.com/bandadoibung.mp3");
				...
				if(PlayerCuffed[i] == 0)
				{
					if (PlayerInfo[i][pHunger] == 0 && ++PlayerInfo[i][pHungerDeathTimer] >= 300) // 5 phut de di an
					{
						...
						SetPVarInt(i, "XIUTAICHO", 2);
						xiutaicho[i] = 1;
						xiutaicho[i] = SetTimerEx("XIUTAICHO", THOIGIANXIU*1000, 0,"d", i);
						PlayerInfo[i][pHungerDeathTimer] = 150;
						ApplyAnimation(i, "CRACK", "crckdeth2", 4.0999, 1, 0, 0, 0, 0);
					}
				}
			}
			if (PlayerInfo[i][pHospital] == 0 && GetPVarInt(i, "Injured") != 1 && GetPVarInt(i, "IsFrozen") == 0 && GetPVarInt(i, "PlayerCuffed") == 0)
			{
				if (++PlayerInfo[i][pThieuNuocTimer] >= 1200 && PlayerInfo[i][pHunger] > 0) // 72 giay la bi -2 trang thai, tuc la 100 hunger thi 1 tieng moi doi bung
				{
					PlayerInfo[i][pThieuNuocTimer] = 0;
					PlayerInfo[i][pKhatNuoc] -= 1;
					if (PlayerInfo[i][pKhatNuoc] < 0)
						PlayerInfo[i][pKhatNuoc] = 0;
					...
					{
						PlayAudioStreamForPlayer(i, "http://web.samp-vn.com/bandadoibung.mp3");
						SendClientMessageEx(i, COLOR_RED, "Ban dang khat nuoc - ban can phai uong nuoc!");
					}
				}

				if(PlayerCuffed[i] == 0)
				{
					if (PlayerInfo[i][pKhatNuoc] == 0 && ++PlayerInfo[i][pKhatNuocTimer] >= 300) // 5 phut de di an
					{
						TogglePlayerControllable(i, 0);
			...
			if(_hungerTextVisible[i] == 1)
			{
				new str[128];
				format(str, sizeof(str), "%d%",PlayerInfo[i][pHunger]);
				PlayerTextDrawSetString(i,StatusFood[i][2],str);
			}
			// update hunger text draw
		/*	switch (PlayerInfo[i][pHunger])
			{
				case 80..100:
					PlayerTextDrawSetString(i, _hungerText[i], "~n~");
				case 40..79:
					PlayerTextDrawSetString(i, _hungerText1[i], "~n~");
				case 20..39:
					PlayerTextDrawSetString(i, _hungerText2[i], "~n~");
				case 0..19:
					PlayerTextDrawSetString(i, _hungerText3[i], "~n~");
			}*/
		}
...
task ServerHeartbeatTwo[1000]() {
	...
	foreach(new i: Player)
	{
		    ...
		    if(GetPVarType(i, "ArmorCheckAgain"))
		    {
		    	if(gettime()-GetPVarInt(i, "ArmorCheckAgain") > 10)
		    	{
 					if(gettime()-GetPVarInt(i, "ArmorWarningTime") > 300)
					{
						...
						new string[128];
					    format( string, sizeof( string ), "[GORP-Anticheats] %s(%d)(health)(Base: %f - Current: %f)", GetPlayerNameEx(i), i, armor, CurrentArmor[i]);
						ABroadCast(COLOR_LIGHTRED, string, 2 );
						format(string, sizeof(string), "[GORP-Anticheats] %s(%d)(health)(Base: %f - Current: %f)", GetPlayerNameEx(i), i, armor, CurrentArmor[i]);
						Log("logs/hack.log", string);
					}
		...
		if(CellTime[i] == 0 && CallCost[i] > 0)
		{
			...
			PlayerInfo[i][pMoneyPhone] -= CallCost[i];
